...
USB_OTG_STS USB_OTG_CoreInit(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  {
    USB_OTG_WRITE_REG32 (&pdev->regs.GREGS->GUSBCFG, 0x00001408);
...
void USB_OTG_InitFSLSPClkSel(USB_OTG_CORE_HANDLE *pdev , uint8_t freq)
{
  ...
  USB_OTG_WRITE_REG32(&pdev->regs.HREGS->HCFG, hcfg.d32);
  if (freq == HCFG_30_60_MHZ)
  {
    USB_OTG_WRITE_REG32(&pdev->regs.HREGS->HFIR, 0x7530);
  }
  else
  {
    USB_OTG_WRITE_REG32(&pdev->regs.HREGS->HFIR, 0xBB80);
  }
}
...
*/
uint32_t USB_OTG_READ_REG32(__IO uint32_t *reg)
{
  uint32_t tmpreg;
