                 ...
                 mbedtls_ecp_restart_ctx *rs_ctx ) /* TBD: rs_ctx is not used MBEDTLS_ECP_RESTARTABLE is not supported */
{
  ...
  if (size == 32)
  { 
    CASPER_ECC_SECP256R1_Mul((CASPER_Type*)CASPER_START_ADDR, &X[0], &Y[0], &X[0], &Y[0], (void *)M);
  }

  if (size == 48)
  {  
    CASPER_ECC_SECP384R1_Mul((CASPER_Type*)CASPER_START_ADDR, &X[0], &Y[0], &X[0], &Y[0], (void *)M);
  }
  if (size == 66)
  {  
    CASPER_ECC_SECP521R1_Mul((CASPER_Type*)CASPER_START_ADDR, &X[0], &Y[0], &X[0], &Y[0], (void *)M);
  }
             ...
             mbedtls_ecp_restart_ctx *rs_ctx ) /* TBD restartable is not implemented */
{
  ...
  if (size == 32)
  { 
    CASPER_ECC_SECP256R1_MulAdd((CASPER_Type*)CASPER_START_ADDR, &X1[0], &Y1[0], &X1[0], &Y1[0], (void *)M, &X2[0], &Y2[0], (void *)N);
  }

  if (size == 48)
  { 
    CASPER_ECC_SECP384R1_MulAdd((CASPER_Type*)CASPER_START_ADDR, &X1[0], &Y1[0], &X1[0], &Y1[0], (void *)M, &X2[0], &Y2[0], (void *)N);
  }
  if (size == 66)
  { 
    CASPER_ECC_SECP521R1_MulAdd((CASPER_Type*)CASPER_START_ADDR, &X1[0], &Y1[0], &X1[0], &Y1[0], (void *)M, &X2[0], &Y2[0], (void *)N);
  }
  /* Reverse results back to MbedTLS format */
  reverse_array((uint8_t *)X1, size);
  reverse_array((uint8_t *)Y1, size);
