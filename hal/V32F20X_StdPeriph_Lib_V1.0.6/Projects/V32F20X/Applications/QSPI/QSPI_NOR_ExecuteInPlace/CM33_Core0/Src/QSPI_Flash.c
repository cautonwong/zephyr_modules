/**
  ...
  * @version V1.0
  * @date    2022-03
  * @brief   This file provides operation functions for windbond W25Q256FV
******************************************************************************/
...
void QSPI_Flash_Init(OSPI_Type *OSPIx)
{
  ...
  if(OSPIx == OSPI0)
  {
    ...
    SYSCFG0_Periph0Reset(SYSCFG0_Periph0Reset_OSPI0);
    SYSCFG0_OSPIClockConfig(OSPI0, SYSCFG0_OSPI_CLKSRC_PLL0, 2);   /* OSPI hclk = PLL0 frequency/2*/
  ...
  else if(OSPIx == OSPI1)
  {
    ...
    GPIO_Init(GPIOG, &GPIO_InitStruct);
    SYSCFG0_Periph0Reset(SYSCFG0_Periph0Reset_OSPI1);
    SYSCFG0_OSPIClockConfig(OSPI1, SYSCFG0_OSPI_CLKSRC_PLL0, 2);   /* OSPI hclk = PLL0 frequency/2*/
...
void QSPI_Flash_QPIMode_SectorErase(OSPI_Type *OSPIx, uint32_t SectorAddr)
{
  ...
  OSPI_InitStructType.OSPI_DataNumber            = 0;
  OSPI_InitStructType.OSPI_WaitCycles            = 0;
  OSPI_InitStructType.OSPI_SCLKDivider           = CLK_GetHCLK0Freq()/1000000;
  OSPI_Init(OSPIx, &OSPI_InitStructType);
...
/**
  ...
  *         DMA only supports 32-bit transfer size.
  * @param  OSPIx: where x can be (0,1) to select OSPI controller
...
void QSPI_Flash_QPIMode_PageWriteWithDMA(OSPI_Type *OSPIx, uint32_t PageWriteAddr, uint8_t *Buff, uint32_t BuffLen)
{
  ...
  for(i = 0; i + 4 <= BuffLen; i += 4)
  {
    ...
    word_buffer[2+i/4] = (Buff[i]<<24)|(Buff[i+1]<<16)|(Buff[i+2]<<8)|Buff[i+3];
  }
...
/**
  * @brief  Configures XIP read in QPI mode
  * @param  OSPIx: where x can be (0,1) to select OSPI controller
  * @retval none
  */
void QSPI_XIP_ReadConfig(OSPI_Type *OSPIx)
{
  ...
  OSPI_InitStructType.OSPI_WaitCycles            = 3;  /* 2 clocks in winbond data sheet, 
                                                        increase it to 3 clocks because of high OSPI interface frequency */
  OSPI_InitStructType.OSPI_SCLKDivider           = 2; 
  OSPI_Init(OSPIx, &OSPI_InitStructType);
  

  OSPIx->SPI_CTRLR0 |= (OSPI_SPI_CTRLR0_XIP_INST_EN |OSPI_SPI_CTRLR0_XIP_DFS_HC);
  OSPIx->XIP_INCR_INST = W25QX_CMD_FAST_READ;
