...
extern __IO unsigned gAB[4096 / 32] __attribute__((section(CASPER_STR_ADDR)));
extern __IO unsigned gCD[64 * 256 / 32] __attribute__((section(CASPER_STR_ADDR)));
extern __IO unsigned gResult[(64 * 256 / 32) + 2] __attribute__((section(CASPER_STR_ADDR)));
...
#define STD_FEATURE_CASPER_RAM_BASE_ADDRESS (CASPER_START_ADDR)
#define CASPER_RAM_BASE_NS (STD_FEATURE_CASPER_RAM_BASE_ADDRESS)
...
int RSA_SignatureToPlaintextFast(const unsigned signature[N_wordlen_max],
                                 size_t sigLen,
                                 const unsigned exp_pubkey[N_wordlen_max],
                                 size_t pubELen,
...
//Calc CIC Setting Value
uint32_t Accel_CIC_Config(CIC_InitType* CIC_InitStruct)
{
  uint32_t tmp = 0;

  tmp |= CIC_CHANNEL_NUM(CIC_InitStruct->ChannelNum);
  tmp |= CIC_RESULT_LENGTH(CIC_InitStruct->ResultLenth);
  tmp |= CIC_INTEGRATE_TYPE(CIC_InitStruct->IntegrateType);
  tmp |= CIC_DECIMATION_OFFSET(CIC_InitStruct->DecimationOffset);
  tmp |= CIC_DECIMATION_RATE(CIC_InitStruct->DecimationRate);
  tmp |= CIC_ORDER(CIC_InitStruct->Order);
...
{
  return CASPER_CTRL_ITER(iter) | CASPER_CTRL_MODE(opcode) | (resAddr << 16);
}
...
void MultprecModulo(unsigned r_out[], const unsigned v[], int top)
{
  uint64_t u64 = 0;                      /*  use 64 bit math mixed with 32 bit */
  unsigned u32 = 0;                      /*  allows us to work on U in 32 bit */
  unsigned u_n = 0, ul16 = 0, uh16 = 0, *u_shft = NULL; /*  u_shft is because r_out is u initially */
  unsigned vl16 = 0, vh16 = 0, v_Nm1 = 0;
  unsigned q_hat = 0, r_hat = 0, q_over = 0;
  unsigned borrow = 0, carry = 0;
  uint32_t i = 0;
  int j = 0, tmp = 0;
    ...
    unsigned w_out[], const unsigned a[], const unsigned b[], const unsigned Nmod[], const unsigned *Np)
{
  ...
  uint64_t Np64 = 0;
  uint64_t carry = 0;
  uint64_t *a64 = NULL, *b64 = NULL, *w64 = NULL, *N64 = NULL;
                                     ...
                                     unsigned MsgRet[WORK_BUFF_MUL4])
{
  ...
  j = pubELen - 1;
  //bitpos = (int8_t)(uint8_t)(31U - __CLZ(exp_pubkey[0])); /*  count of bits after the left most 1 */
  bitpos = (pubELen * 32U - __CLZ(exp_pubkey[j]) - 1); /*  count of bits after the left most 1 */
  i = __CLZ(exp_pubkey[j]) + 1;
  m = 31 - i;
  j = pubELen - 1;
  while (--bitpos >= 0)
  {
    ...
    i = exp_pubkey[j] & (uint32_t)(1U << (uint8_t)m);
    if (0U != i) /*  where e is 1 */
    {
    ...
    {
//      MultprecCiosMul(&MsgRet[kCASPER_RamOffset_M64],
//                      &MsgRet[kCASPER_RamOffset_Result],
//                      &MsgRet[(bidx != 0) ? kCASPER_RamOffset_Base : kCASPER_RamOffset_TempBase], pubkey, np64);
                                 ...
                                 unsigned MsgRet[WORK_BUFF_MUL4])
{
  ...
  MultprecMontCalcRp(&MsgRet[kCASPER_RamOffset_Result], exp_pubkey[0], pubkey); /*  calculate R` (=R mod N) */
  MultprecMontPrepareX(&MsgRet[kCASPER_RamOffset_Base], signature, &MsgRet[kCASPER_RamOffset_Result],
                       pubkey); /*  X*R1` mod N */
  return (RSA_MontSignatureToPlaintextFast(&MsgRet[kCASPER_RamOffset_Base], exp_pubkey, pubELen, pubkey, MsgRet));
}
 ...
 */
// base = signature ^ pubE MOD pubN
void CASPER_ModExp(
    CASPER_Type *base, const uint8_t *signature, size_t signatureWordLen, const uint8_t *pubN, size_t wordLen, const uint8_t * pubE, size_t pubELen, uint8_t *plaintext)
{
#define PK_LOC  &msg_ret[kCASPER_RamOffset_Modulus]
#define SIG_LOC &msg_ret[(unsigned)kCASPER_RamOffset_Modulus + N_wordlen_max]
#define PKE_LOC &msg_ret[(unsigned)kCASPER_RamOffset_Modulus + N_wordlen_max + N_wordlen_max]
  ...
  CASPER_MEMCPY_N2I(SIG_LOC, (const uint32_t *)(uintptr_t)signature, (signatureWordLen << 2));
  CASPER_MEMCPY_N2I(PKE_LOC, (const uint32_t *)(uintptr_t)pubE, (pubELen << 2));
  (void)RSA_SignatureToPlaintextFast((const unsigned *)(uintptr_t)(SIG_LOC),signatureWordLen, PKE_LOC, pubELen, 
...
void CASPER_Init(uint32_t base)
{
  ...
  /* If Casper init is called with secure address, use secure addres also for accessing Casper RAM. */
	/* For Mars+ ,using shared RAM */
  //Got Private Space
  s_casperRamBase = (unsigned)CASPER_RAM_BASE_NS;;
  msg_ret         = (uint32_t *)s_casperRamBase;
	

  //Enable CoProcessor via CM33 command
	SCB->CPACR      |= (3UL << 2);
	while(!(SCB->CPACR & (3UL << 2))){}
	Accel_SetSRAMBaseAddr(base);
}
...
void CASPER_Deinit(CASPER_Type *base)
{
  ...
  //Disable CoProcessor via CM33 command
	SCB->CPACR      &= ~(3UL << 2);
	while((SCB->CPACR & (3UL << 2))){}

  SYSCFG0_Periph0ClockCmd(SYSCFG0_Periph0Clock_CRYPTCOP, DISABLE);
}
    ...
    uint32_t w_out[], const uint32_t a[], const uint32_t b[], const uint32_t Nmod[], const uint32_t *Np)
{
  ...
  uint64_t carry = 0;
  uint64_t *a64 = NULL, *b64 = NULL, *w64 = NULL, *N64 = NULL;
  uint32_t *T1 = &CASPER_MEM[0], borrow = 0;
...
static void CASPER_montsub(uint32_t *C, uint32_t *A, uint32_t *B, uint32_t *mod)
{
  uint64_t *b64 = NULL, *c64 = NULL, *m64 = NULL, *tmp = NULL;
  int borrow = 0;
...
void Jac_addition(uint32_t *X3,
                  ...
                  uint32_t *Z2)
{
  uint32_t *Z1Z1 = NULL, *Z2Z2 = NULL, *U1 = NULL, *S1 = NULL, *J = NULL, *H = NULL, *V = NULL, *t0 = NULL, *t1 = NULL;
  int m1 = 0, m2 = 0;
...
void Jac_add_affine(
    uint32_t *X3, uint32_t *Y3, uint32_t *Z3, uint32_t *X1, uint32_t *Y1, uint32_t *Z1, uint32_t *X2, uint32_t *Y2)
{
  uint32_t *T1 = NULL, *T2 = NULL, *T3 = NULL, *T4 = NULL, *T5 = NULL;
  uint32_t *ONE = NULL;
  int m1 = 0, m2 = 0;
...
void Jac_scalar_multiplication(
    uint32_t *X3, uint32_t *Y3, uint32_t *Z3, uint32_t *X1, uint32_t *Y1, uint32_t *k, uint32_t *p, uint32_t *q)
{
  uint32_t *scalar = NULL, *M = NULL, *X = NULL, *Y = NULL, *Z = NULL, *mem = NULL;
  uint32_t *ONE = NULL;
  int i = 0, sign = 0, odd = 0;
  uint8_t index = 0;
...
{
  uint32_t *Q2x = NULL, *Q2y = NULL, *Q2z = NULL, *P2x = NULL, *P2y = NULL, *P2z = NULL, *Z = NULL, *mem = NULL;
  uint32_t *ONE  = NULL;
  uint32_t index = 0;
...
{
  uint32_t *Z = NULL, *mem = NULL, *ONE = NULL;
  uint32_t index = 0;
                                  ...
                                  uint32_t *k2)
{
  ...
  uint32_t *p1 = NULL, *p2 = NULL, x1 = 0, x2 = 0, *lut = NULL, *Tx = NULL, *Ty = NULL, *Tz = NULL;
  size_t bitlen = 0, shiftr = 0, shiftl = 0u;
...
{
  int i = 0;
  uint32_t *t = NULL, *t2 = NULL, *s1 = NULL, *s2 = NULL, *s4 = NULL, *s8 = NULL, *tmp = NULL;
  ...
  // t2 = n^(2^1)*n  # 11
  square_casper(tmp, a);
  multiply_casper(t2, tmp, a); // 3

  // s1 = t2^(2^2)*t2  # F
  square_casper(s1, t2);  // 6
  square_casper(tmp, s1); // 12
  multiply_casper(s1, tmp, t2); // 15
...
{
  int i = 0;
  uint32_t *e = NULL, *d = NULL, *tmp = NULL, *t0 = NULL, *t1 = NULL;
  uint32_t  *t2 = NULL, *t3 = NULL, *t4 = NULL, *t5 = NULL, *t6 = NULL; // 10 residues needed
  ...
  // t1 = t0^(2^4)*t0     # ff
  square_casper(tmp, t0); // 30
  square_casper(t1, tmp); // 60
  square_casper(tmp, t1); // 120
  square_casper(t2, tmp); // 240
  multiply_casper(t1, t2, t0); // 255

  // t2 = t1^(2^8)*t1   # 4f
  square_casper(tmp, t1); // 500
  for (i = 0; i < 3; i++)
  {
    square_casper(t3, tmp); // 1000
    square_casper(tmp, t3); // 2000
  }
...
static void toMontgomery_ECC_P384(uint32_t *C, uint32_t *A)
{
  ...
  uint32_t *T1 = NULL, *T2 = NULL, *T3 = NULL;
...
{
  int i = 0;
  uint32_t *e3 = NULL, *d2 = NULL, *d3 = NULL, *d4 = NULL, *T2 = NULL, *T4 = NULL; // 6 residues needed
  ...
  square_casper(d2, a);         // 2
  multiply_casper(T2, d2, a);   // 3

  // d3 = 2^2 * T2
  square_casper(d3, T2);       // 6
  square_casper(e3, d3);       // 12
  multiply_casper(T4, e3, T2); // 15

  // d3 = 2^4 * T4
  square_casper(d3, T4);      // 30     
  square_casper(e3, d3);      // 60
  square_casper(d3, e3);      // 120
  square_casper(e3, d3);      // 240
  multiply_casper(d2, e3, T4); // 255

  // d3 = 2^8 * d2
  square_casper(d3, d2); // 500
  square_casper(e3, d3); // 100
  for (i = 0; i < 3; i++)
  {
    square_casper(d3, e3);
    square_casper(e3, d3);
    ...
    uint32_t w_out[], const uint32_t a[], const uint32_t b[], const uint32_t Nmod[], const uint32_t *Np)
{
  uint32_t j = 0;
  uint64_t carry = 0;
  uint64_t *a64 = NULL, *b64 = NULL, *w64 = NULL;
...
{
//  uint32_t* PA = &msg_ret[150];
//  uint32_t* PB = &msg_ret[150+150];
//  uint32_t* PC = &msg_ret[150+150+150];
 // uint32_t i = 0;
...
//  }
 ...
 // CASPER_MEMCPY_N2I(PB, (const uint32_t *)(uintptr_t)B, (sizeB << 2));

  /*  R is 2^n where n is 1 bit longer than Nmod, so 1 followed by 32 or 64 0 words for example */
  /*  Note that Nmod's upper most bit has to be 1 by definition, so one subtract is enough. We */
  /*  do not set the 1 since it is "borrowed" so no point */
  
 // CASPER_MEMCPY_N2I(PC, (const uint32_t *)(uintptr_t)A, (sizeA << 2));
  Accel_SetABCD_Addr(CA_MK_OFF(B), 0);
  Accel_crypto_mul(Accel_IterOpcodeResaddr((sizeA >> 1) - 1U, (uint32_t)kCASPER_OpSub64, CA_MK_OFF(C)));
  Accel_done();
  //CASPER_MEMCPY_N2I(C, (const uint32_t *)(uintptr_t)C, (sizeA << 2));
  return 0;
}
...
int CASPER_ModMul(uint8_t *A, uint32_t sizeA, uint8_t *B, uint32_t sizeB, uint8_t *C)
{
  ...
  /*  We do 1st pass NOSUM so we do not have to 0 output */
  Accel_SetABCD_Addr(CA_MK_OFF(PB), CA_MK_OFF(PA));
  Accel_crypto_mul(
    Accel_IterOpcodeResaddr((sizeA >> 1) - 1U, (uint32_t)kCASPER_OpMul6464NoSum, CA_MK_OFF(PC)));
  Accel_done();
  /*  Step 2. iterate over N words of v using j */
  for (j = 2U; j < sizeB; j += 2U)
  {
    /*  Step 2b. Check for 0 on v word - skip if so since we 0ed already */
    /*  Step 3. Iterate over N words of u using i - perform Multiply-accumulate */
    if (0U != (GET_WORD(&PB[j])) || 0U != (GET_WORD(&PB[j + 1U])))
    {
      Accel_SetABCD_Addr(CA_MK_OFF(&PB[j]), CA_MK_OFF(PA));
      Accel_crypto_mul(
          Accel_IterOpcodeResaddr((sizeA >> 1)  - 1U, (uint32_t)kCASPER_OpMul6464Sum, CA_MK_OFF(&PC[j])));
      Accel_done();
    }
  }
...
int CASPER_ModAdd(uint8_t *A, uint32_t sizeA, uint8_t *B, uint32_t sizeB, uint8_t *C)
{
 ...
 // CASPER_MEMCPY_N2I(PC, (const uint32_t *)(uintptr_t)A, (sizeA << 2));
  Accel_SetABCD_Addr(CA_MK_OFF(B), 0);
  Accel_crypto_mul(Accel_IterOpcodeResaddr((sizeA >> 1), (uint32_t)kCASPER_OpAdd64, CA_MK_OFF(C)));
  Accel_done();
  //CASPER_MEMCPY_N2I(C, (const uint32_t *)(uintptr_t)C, (sizeA << 2));
  return 0;
}
