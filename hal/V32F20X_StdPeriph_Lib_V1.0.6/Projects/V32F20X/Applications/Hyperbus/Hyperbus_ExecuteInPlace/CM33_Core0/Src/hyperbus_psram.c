...
void HyperbusPSRAM_InitHyperbus(OSPI_Type* OSPIx)
{
  ...
  if(OSPIx == OSPI0)
  {
    ...
    SYSCFG0_OSPIClockConfig(OSPI0, SYSCFG0_OSPI_CLKSRC_PLL0, 1);
  ...
  else if(OSPIx == OSPI1)
  {
    ...
    SYSCFG0_OSPIClockConfig(OSPI1, SYSCFG0_OSPI_CLKSRC_PLL0, 1);
...
/**
  * @brief  Writes data to PSRAM in XIP mode
  * @notice This function demands that NumByteToWrite is multiple of 4 bytes for simplicity.
...
void HyperbusPSRAM_WritePSRAM(OSPI_Type* OSPIx, uint8_t *pBuffer, uint32_t WriteAddr, uint32_t NumByteToWrite)
{
  ...
  if(NumByteToWrite == 0 || NumByteToWrite%4 != 0)
  {
    return;
  }
  ...
  {
    *(uint32_t*)(WriteAddr + i) = *(uint32_t*)(pBuffer + i); 
  }
...
/**
  * @brief  Reads data From PSRAM in XIP mode
...
void HyperbusPSRAM_ReadPSRAM(OSPI_Type* OSPIx, uint32_t* pBuffer, uint32_t ReadAddr, uint32_t NumByteToRead)
{
  ...
  {
    pBuffer[i/4] = *(uint32_t*)(ReadAddr + i);
  }
