...
{  
  ...
  printf("Start lwip init\r\n");
  while(lwip_comm_init()!=0);
    
  if(ETH0 == USER_ETH)
  {
    NVIC_SetPriority(MAC0_SBD_IRQn, NVIC_EncodePriority(NVIC_GetPriorityGrouping(), 5, 0));
    NVIC_EnableIRQ(MAC0_SBD_IRQn);
  }
  else
  {
    NVIC_SetPriority(MAC1_SBD_IRQn, NVIC_EncodePriority(NVIC_GetPriorityGrouping(), 5, 0));
