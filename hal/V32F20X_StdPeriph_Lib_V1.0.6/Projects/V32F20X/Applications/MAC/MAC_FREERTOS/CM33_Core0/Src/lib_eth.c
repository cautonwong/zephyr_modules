...
__IO ETH_DMADESCTypeDef  *DMATxDescToSetEth1;
__IO ETH_DMADESCTypeDef  *DMARxDescToGetEth1;
...
void ETH_DeInit(ETH_TypeDef* ETHx)
{
  if (ETH0 == ETHx)
  {
    SYSCFG0_Periph0Reset(SYSCFG0_Periph0Reset_MAC0);
  }
  else
  {
    SYSCFG0_Periph0Reset(SYSCFG0_Periph0Reset_MAC1);
  }
} 
...
uint32_t ETH_Init(ETH_TypeDef* ETHx, ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  ...
  tmpreg = ETHx->MACMIIAR;
  /* Clear CSR Clock Range CR[2:0] bits */
  tmpreg &= MACMIIAR_CR_MASK;
  /* Get hclk frequency value */
  hclk=ETH_InitStruct->Sys_Clock_Freq;
  ...
  ETHx->MACMIIAR = (uint32_t)tmpreg;  
  /*-------------------- PHY initialization and configuration ----------------*/
  /* Put the PHY in reset mode */
  if(!(ETH_WritePHYRegister(ETHx, PHYAddress, PHY_BCR, PHY_Reset)))
  {
  ...
  {  
    ...
    {
      timeout++;
    } while (!(ETH_ReadPHYRegister(ETHx, PHYAddress, PHY_BSR) & PHY_Linked_Status) && (timeout < PHY_READ_TO));
    ...
    // Flow control
    ret = ETH_ReadPHYRegister(ETHx, PHYAddress, 0x04);
    ret |= 0x0400;
    ETH_WritePHYRegister(ETHx, PHYAddress, 0x04, ret);

    /* Reset Timeout counter */
    timeout = 0; 
    /* Enable Auto-Negotiation */
    if(!(ETH_WritePHYRegister(ETHx, PHYAddress, PHY_BCR, PHY_AutoNegotiation)))
    {
      /* Return ERROR in case of write timeout */
      return ETH_ERROR;
    }
    ...
    {
      timeout++;
    } while (!(ETH_ReadPHYRegister(ETHx, PHYAddress, PHY_BSR) & PHY_AutoNego_Complete) && (timeout < (uint32_t)PHY_READ_TO));  
    ...
    RegValue = ETH_ReadPHYRegister(ETHx, PHYAddress, PHY_SR);
  ...
  {
      if (ETH_Speed_100M == ETH_InitStruct->ETH_Speed)
      {
          _eth_delay_(PHY_SET_MODE_DELAY);
      }
      
    if(!ETH_WritePHYRegister(ETHx, PHYAddress, PHY_BCR, ((uint16_t)(ETH_InitStruct->ETH_Mode >> 3) |
  ...
  ETHx->MACFFR = (uint32_t)(ETH_InitStruct->ETH_ReceiveAll | 
  ...
  ETHx->MACHTHR = (uint32_t)ETH_InitStruct->ETH_HashTableHigh;
  /* Write to ETHERNET MACHTLR */
  ETHx->MACHTLR = (uint32_t)ETH_InitStruct->ETH_HashTableLow;
  ...
  ETHx->MACFCR = (uint32_t)tmpreg;
  /*----------------------- ETHERNET MACVLANTR Configuration -----------------*/
  /* Set the ETV bit according to ETH_VLANTagComparison value */
  /* Set the VL bit according to ETH_VLANTagIdentifier value */  
  ETHx->MACVLANTR = (uint32_t)(ETH_InitStruct->ETH_VLANTagComparison | 
  ...
  ETHx->DMABMR = (uint32_t)(ETH_InitStruct->ETH_AddressAlignedBeats | 
...
void ETH_Start(ETH_TypeDef* ETHx)
{
  ...
  ETH_MACTransmissionCmd(ETHx, ENABLE);
  /* Flush Transmit FIFO */
  ETH_FlushTransmitFIFO(ETHx);
  /* Enable receive state machine of the MAC for reception from the MII */  
  ETH_MACReceptionCmd(ETHx,ENABLE);
 
  /* Start DMA transmission */
  ETH_DMATransmissionCmd(ETHx, ENABLE); 
  /* Start DMA reception */
  ETH_DMAReceptionCmd(ETHx, ENABLE);   
}
...
FlagStatus ETH_GetFlowControlBusyStatus(ETH_TypeDef* ETHx)
{
  ...
  if ((ETHx->MACFCR & ETH_MACFCR_FCBBPA) != (uint32_t)RESET)
  {
    bitstatus = SET;
  }
  else
...
FlagStatus ETH_GetMACFlagStatus(ETH_TypeDef* ETHx, uint32_t ETH_MAC_FLAG)
{
  ...
  if ((ETHx->MACSR & ETH_MAC_FLAG) != (uint32_t)RESET)
  {
    bitstatus = SET;
  }
  else
...
/**
  ...
  * @param  ETH_MAC_INT: specifies the interrupt source to check.
  *   This parameter can be one of the following values:
...
ITStatus ETH_GetMACINTStatus(ETH_TypeDef* ETHx, uint32_t ETH_MAC_INT)
{
  ...
  /* Check the parameters */
  assert_param(IS_ETH_MAC_GET_INT(ETH_MAC_INT)); 
  if ((ETHx->MACSR & ETH_MAC_INT) != (uint32_t)RESET)
  {
    bitstatus = SET;
  }
  else
...
/**
  ...
  * @param  ETH_MAC_INT: specifies the ETHERNET MAC interrupt sources to be
  *   enabled or disabled.
...
{
  /* Check the parameters */
  assert_param(IS_ETH_MAC_INT(ETH_MAC_INT));
  assert_param(IS_FUNCTIONAL_STATE(NewState));  
  
  if (NewState != DISABLE)
  {
    /* Enable the selected ETHERNET MAC interrupts */
    ETHx->MACIMR &= (~(uint32_t)ETH_MAC_INT);
  }
  else
  {
    /* Disable the selected ETHERNET MAC interrupts */
    ETHx->MACIMR |= ETH_MAC_INT;
  }
}
...
/**
  ...
  *     @arg ETH_MAC_Address4 : MAC Address4
  *     @arg ETH_MAC_Address5 : MAC Address5
  *     @arg ETH_MAC_Address6 : MAC Address6
  *     @arg ETH_MAC_Address7 : MAC Address7
...
void ETH_MACAddressConfig(ETH_TypeDef* ETHx, uint32_t MacAddr, uint8_t *Addr)
{
  ...
  if (ETH0 == ETHx)
  {
    base_h = ETH_MAC0_ADDR_HBASE;
    base_l = ETH_MAC0_ADDR_LBASE;
  }
  else
  {
    base_h = ETH_MAC1_ADDR_HBASE;
    base_l = ETH_MAC1_ADDR_LBASE;
  }
  /* Check the parameters */
  assert_param(IS_ETH_MAC_ADDRESS01234567(MacAddr));
  
  /* Calculate the selected MAC address high register */
  tmpreg = ((uint32_t)Addr[5] << 8) | (uint32_t)Addr[4]; 
  /* Load the selected MAC address high register */
  (*(__IO uint32_t *) (base_h + MacAddr)) = tmpreg; 
  /* Calculate the selected MAC address low register */
  tmpreg = ((uint32_t)Addr[3] << 24) | ((uint32_t)Addr[2] << 16) | ((uint32_t)Addr[1] << 8) | Addr[0];
 
  /* Load the selected MAC address low register */
  (*(__IO uint32_t *) (base_l + MacAddr)) = tmpreg;
}
...
void ETH_MACAddressPerfectFilterCmd(ETH_TypeDef* ETHx, uint32_t MacAddr, FunctionalState NewState)
{
  ...
  /* Check the parameters */
  assert_param(IS_ETH_MAC_ADDRESS1234567(MacAddr));
  assert_param(IS_FUNCTIONAL_STATE(NewState));
  ...
  if (NewState != DISABLE)
  {
    ...
    (*(__IO uint32_t *) (base_h + MacAddr)) |= ETH_MACA1HR_AE;
  }
  else
  {
    /* Disable the selected ETHERNET MAC address for perfect filtering */
    (*(__IO uint32_t *) (base_h + MacAddr)) &=(~(uint32_t)ETH_MACA1HR_AE);
  }
}
...
void ETH_MACAddressFilterConfig(ETH_TypeDef* ETHx, uint32_t MacAddr, uint32_t Filter)
{
  ...
  if (Filter != ETH_MAC_AddressFilter_DA)
  {
    /* The selected ETHERNET MAC address is used to compare with the SA fields of the
       received frame. */
    (*(__IO uint32_t *) (base_h + MacAddr)) |= ETH_MACA1HR_SA;
  }
  else
  {
    /* The selected ETHERNET MAC address is used to compare with the DA fields of the
       received frame. */
    (*(__IO uint32_t *) (base_h + MacAddr)) &=(~(uint32_t)ETH_MACA1HR_SA);
  }
}
...
void ETH_MACAddressMaskBytesFilterConfig(ETH_TypeDef* ETHx, uint32_t MacAddr, uint32_t MaskByte)
{
  ...
  (*(__IO uint32_t *) (base_h + MacAddr)) &=(~(uint32_t)ETH_MACA1HR_MBC);
  /* Set the selected Filter mask bytes */
  (*(__IO uint32_t *) (base_h + MacAddr)) |= MaskByte;
}
...
{   
  ...
  if ((ETHx->DMASR & ETH_DMASR_TBUS) != (u32)RESET)
  {
    /* Clear TBUS ETHERNET DMA flag */
    ETHx->DMASR = ETH_DMASR_TBUS;
...
void ETH_DMARxDescChainInit(ETH_TypeDef* ETHx, ETH_DMADESCTypeDef *DMARxDescTab, uint8_t *RxBuff, uint32_t RxBuffCount)
{
  ...
  ETHx->DMARDLAR = (uint32_t) DMARxDescTab; 
...
void ETH_DMATxDescChainInit(ETH_TypeDef* ETHx, ETH_DMADESCTypeDef *DMATxDescTab, uint8_t* TxBuff, uint32_t TxBuffCount)
{
  ...
  ETHx->DMATDLAR = (uint32_t) DMATxDescTab;
}
...
/**
  ...
  * @retval None
  */
void ETH_DMATxDescTransmitINTConfig(ETH_DMADESCTypeDef *DMATxDesc, FunctionalState NewState)
{
  /* Check the parameters */
  assert_param(IS_FUNCTIONAL_STATE(NewState));
...
{
  FlagStatus bitstatus = RESET;
  if((ETHx->DMABMR & ETH_DMABMR_SR) != (uint32_t)RESET)
  {
    bitstatus = SET;
  }
  else
...
{  
  ...
  if ((ETHx->DMASR & ETH_DMA_FLAG) != (uint32_t)RESET)
  {
    bitstatus = SET;
  }
  else
...
void ETH_DMAClearFlag(ETH_TypeDef* ETHx, uint32_t ETH_DMA_FLAG)
{
  ...
  ETHx->DMASR = (uint32_t) ETH_DMA_FLAG;
}

/**
  * @brief  Enables or disables the specified ETHERNET DMA interrupts.
  * @param  ETH_DMA_INT: specifies the ETHERNET DMA interrupt sources to be
  *   enabled or disabled.
  *   This parameter can be any combination of the following values:
...
{
  /* Check the parameters */
  assert_param(IS_ETH_DMA_INT(ETH_DMA_INT));
  assert_param(IS_FUNCTIONAL_STATE(NewState));  
  
  if (NewState != DISABLE)
  {
    /* Enable the selected ETHERNET DMA interrupts */
    ETHx->DMAIER |= ETH_DMA_INT;
  }
  else
  {
    /* Disable the selected ETHERNET DMA interrupts */
    ETHx->DMAIER &=(~(uint32_t)ETH_DMA_INT);
  }
}

/**
  * @brief  Checks whether the specified ETHERNET DMA interrupt has occurred or not.
  * @param  ETH_DMA_INT: specifies the interrupt source to check.
  *   This parameter can be one of the following values:
  ...
  *     @arg ETH_DMA_INT_TPS : Transmit process stopped interrupt 
  *     @arg ETH_DMA_INT_T   : Transmit interrupt 
  * @retval The new state of ETH_DMA_INT (SET or RESET).
  */
ITStatus ETH_GetDMAINTStatus(ETH_TypeDef* ETHx, uint32_t ETH_DMA_INT)
{  
  ITStatus bitstatus = RESET;
  /* Check the parameters */
  assert_param(IS_ETH_DMA_GET_INT(ETH_DMA_INT));
  if ((ETHx->DMASR & ETH_DMA_INT) != (uint32_t)RESET)
  {
    bitstatus = SET;
  }
  else
...
/**
  * @brief  Clears the ETHERNETs DMA INT pending bit.
  * @param  ETH_DMA_INT: specifies the interrupt pending bit to clear.
  *   This parameter can be any combination of the following values:
  *     @arg ETH_DMA_INT_NIS : Normal interrupt summary 
  *     @arg ETH_DMA_INT_AIS : Abnormal interrupt summary  
  *     @arg ETH_DMA_INT_ER  : Early receive interrupt 
...
void ETH_DMAClearINTPendingBit(ETH_TypeDef* ETHx, uint32_t ETH_DMA_INT)
{
  ...
  /* Clear the selected ETHERNET DMA INT */
  ETHx->DMASR = (uint32_t) ETH_DMA_INT;
}
...
{
  return ((uint32_t)(ETHx->DMASR & ETH_DMASR_TS)); 
}
...
{
  return ((uint32_t)(ETHx->DMASR & ETH_DMASR_RS)); 
}
...
{   
  FlagStatus bitstatus = RESET;
  if ((ETHx->DMAOMR & ETH_DMAOMR_FTF) != (uint32_t)RESET)
  {
    bitstatus = SET;
  }
  else
...
FlagStatus ETH_GetDMAOverflowStatus(ETH_TypeDef* ETHx, uint32_t ETH_DMA_Overflow)
{
  ...
  if ((ETHx->DMAMFBOCR & ETH_DMA_Overflow) != (uint32_t)RESET)
  {
    bitstatus = SET;
  }
  else
...
{
  return ((uint32_t)((ETHx->DMAMFBOCR & ETH_DMAMFBOCR_MFA)>>ETH_DMA_RX_OVERFLOW_MISSEDFRAMES_COUNTERSHIFT));
}
...
{
  return ((uint32_t)(ETHx->DMAMFBOCR) & ETH_DMAMFBOCR_MFC);
}
...
{
  return ((uint32_t)(ETHx->DMACHTBAR));
}
...
{
  return ((uint32_t)(ETHx->DMACHRBAR));
}
...
uint32_t ETH_PHYLoopBackCmd(ETH_TypeDef* ETHx, uint16_t PHYAddress, FunctionalState NewState)
{
  ...
  tmpreg = ETH_ReadPHYRegister(ETHx, PHYAddress, PHY_BCR); 
  ...
  if(ETH_WritePHYRegister(ETHx, PHYAddress, PHY_BCR, tmpreg) != (uint32_t)RESET)
  {
    return ETH_SUCCESS;
  }
  else
...
{  
  ...
  ETHx->MACPMTCSR |= ETH_MACPMTCSR_WFFRPR;  
}
...
{ 
  ...
  if (NewState != DISABLE)
  {
    ...
    ETHx->MACPMTCSR |= ETH_MACPMTCSR_GU;  
  }
  else
  {
...
FlagStatus ETH_GetPMTFlagStatus(ETH_TypeDef* ETHx, uint32_t ETH_PMT_FLAG)
{
  ...
  if ((ETHx->MACPMTCSR & ETH_PMT_FLAG) != (uint32_t)RESET)
  {
    bitstatus = SET;
  }
  else
...
{ 
  ...
  if (NewState != DISABLE)
  {
    ...
    ETHx->MACPMTCSR |= ETH_MACPMTCSR_WFE;  
  }
  else
  {
...
{ 
  ...
  if (NewState != DISABLE)
  {
    ...
    ETHx->MACPMTCSR |= ETH_MACPMTCSR_MPE;  
  }
  else
  {
...
{ 
  ...
  if (NewState != DISABLE)
  {
    ...
    ETHx->MACPMTCSR |= ETH_MACPMTCSR_PD;  
  }
  else
  {
...
void ETH_MMCCounterFullPreset(ETH_TypeDef* ETHx)
{
  ...
  ETHx->MMCCR |= ETH_MMCCR_MCFHP | ETH_MMCCR_MCP;
}
...
void ETH_MMCCounterHalfPreset(ETH_TypeDef* ETHx)
{
  ...
  ETHx->MMCCR &= ~ETH_MMCCR_MCFHP;
  /* Initialize the MMC counters to almost-half value */
  ETHx->MMCCR |= ETH_MMCCR_MCP;
}
...
void ETH_MMCCountersReset(ETH_TypeDef* ETHx)
{
  ...
  ETHx->MMCCR |= ETH_MMCCR_CR; 
}

/**
  * @brief  Enables or disables the specified ETHERNET MMC interrupts.
  * @param  ETH_MMC_INT: specifies the ETHERNET MMC interrupt sources to be enabled or disabled.
...
{ 
  /* Check the parameters */
  assert_param(IS_ETH_MMC_INT(ETH_MMC_INT));  
  assert_param(IS_FUNCTIONAL_STATE(NewState));
   
  if ((ETH_MMC_INT & (uint32_t)0x10000000) != (uint32_t)RESET)
  {
    /* Remove Register mak from INT */
    ETH_MMC_INT &= 0xEFFFFFFF;
  
    /* ETHERNET MMC Rx interrupts selected */
    if (NewState != DISABLE)
    {
      /* Enable the selected ETHERNET MMC interrupts */
      ETHx->MMCRIMR &=(~(uint32_t)ETH_MMC_INT);
    }
    else
    {
      /* Disable the selected ETHERNET MMC interrupts */
      ETHx->MMCRIMR |= ETH_MMC_INT;    
    }
  }
  else
  {
    ...
    {
      /* Enable the selected ETHERNET MMC interrupts */
      ETHx->MMCTIMR &=(~(uint32_t)ETH_MMC_INT);
    }
    else
    {
      /* Disable the selected ETHERNET MMC interrupts */
      ETHx->MMCTIMR |= ETH_MMC_INT;    
    }  
  }
}

/**
  * @brief  Checks whether the specified ETHERNET MMC INT is set or not.
  * @param  ETH_MMC_INT: specifies the ETHERNET MMC interrupt.
  *   This parameter can be one of the following values:
  *     @arg ETH_MMC_INT_TxFCGC: When Tx good frame counter reaches half the maximum value 
  *     @arg ETH_MMC_INT_TxMCGC: When Tx good multi col counter reaches half the maximum value 
  *     @arg ETH_MMC_INT_TxSCGC: When Tx good single col counter reaches half the maximum value 
  *     @arg ETH_MMC_INT_RxUGFC: When Rx good unicast frames counter reaches half the maximum value  
  *     @arg ETH_MMC_INT_RxAEC : When Rx alignment error counter reaches half the maximum value 
  *     @arg ETH_MMC_INT_RxCEC : When Rx crc error counter reaches half the maximum value 
  * @retval The value of ETHERNET MMC INT (SET or RESET).
  */
ITStatus ETH_GetMMCINTStatus(ETH_TypeDef* ETHx, uint32_t ETH_MMC_INT)
{
  ITStatus bitstatus = RESET;
  /* Check the parameters */
  assert_param(IS_ETH_MMC_GET_INT(ETH_MMC_INT)); 
  
  if ((ETH_MMC_INT & (uint32_t)0x10000000) != (uint32_t)RESET)
  {
    /* ETHERNET MMC Rx interrupts selected */
    /* Check if the ETHERNET MMC Rx selected interrupt is enabled and occurred */ 
    if ((((ETHx->MMCRIR & ETH_MMC_INT) != (uint32_t)RESET)) && ((ETHx->MMCRIMR & ETH_MMC_INT) == (uint32_t)RESET))
    {
      bitstatus = SET;
    }
    else
  ...
  else
  {
    ...
    if ((((ETHx->MMCTIR & ETH_MMC_INT) != (uint32_t)RESET)) && ((ETHx->MMCRIMR & ETH_MMC_INT) == (uint32_t)RESET))
    {
      bitstatus = SET;
    }
    else
...
uint32_t ETH_GetMMCRegister(ETH_TypeDef* ETHx, uint32_t ETH_MMCReg)
{
  ...
  return (*(__IO uint32_t *)(ETH_MAC0_BASE + ETH_MMCReg));
}


void ptp_init(ETH_TypeDef* ETHx, PTP_InitTypeDef* PTP_InitStruct)
{
  uint32_t tmpreg = 0;
  
  assert_param(IS_PTP_FRAME_FILTERING(PTP_InitStruct->PTP_FrameFiltering));
  assert_param(IS_ETH_PTP_TYPE_CLOCK(PTP_InitStruct->PTP_ClockNodeType));
  assert_param(IS_ETH_PTP_SNAPSHOT(PTP_InitStruct->PTP_SnapshotForMessage));
  assert_param(IS_ETH_PTP_SNAPSHOT(PTP_InitStruct->PTP_SnapshotForEventMessage));
  assert_param(IS_ETH_PTP_SNAPSHOT(PTP_InitStruct->PTP_SnapshotForIpv4Frames));
  assert_param(IS_ETH_PTP_SNAPSHOT(PTP_InitStruct->PTP_SnapshotForIpv6Frames));
  assert_param(IS_ETH_PTP_SNAPSHOT(PTP_InitStruct->PTP_SnapshotForPtpOverEnthernet));
  assert_param(IS_ETH_PTP_SNAPSHOT(PTP_InitStruct->PTP_SnapshotPtpV2Frames));
  assert_param(IS_PTP_SUBSECOND_ROLLOVER(PTP_InitStruct->PTP_SubsecondRollover));
  assert_param(IS_ETH_PTP_SNAPSHOT(PTP_InitStruct->PTP_SnapshotForAllReceived));
  assert_param(IS_PTP_ADDEND_REGISTER_UPDATE(PTP_InitStruct->PTP_AddendRegisterUpdate));
  assert_param(IS_PTP_INTERRUPT_TRIGGER(PTP_InitStruct->PTP_InterruptTriggerEnable));
  assert_param(IS_PTP_SYSTEM_TIME_UPDATE(PTP_InitStruct->PTP_SystemTimeUpdate));
  assert_param(IS_PTP_SYSTEM_TIME_INIT(PTP_InitStruct->PTP_SystemTimeInit));
  assert_param(IS_PTP_FILE_OR_COARSE_UPDATE(PTP_InitStruct->PTP_FileOrCoarseUpdate));
  assert_param(IS_PTP_TIME_STAMP(PTP_InitStruct->PTP_PtpEnable));


  tmpreg |= (PTP_InitStruct->PTP_FrameFiltering | PTP_InitStruct->PTP_ClockNodeType |
             PTP_InitStruct->PTP_SnapshotForMessage | PTP_InitStruct->PTP_SnapshotForEventMessage |
             PTP_InitStruct->PTP_SnapshotForIpv4Frames | PTP_InitStruct->PTP_SnapshotForIpv6Frames |
             PTP_InitStruct->PTP_SnapshotForPtpOverEnthernet | PTP_InitStruct->PTP_SnapshotPtpV2Frames |
             PTP_InitStruct->PTP_SubsecondRollover | PTP_InitStruct->PTP_SnapshotForAllReceived |
             PTP_InitStruct->PTP_InterruptTriggerEnable | PTP_InitStruct->PTP_FileOrCoarseUpdate |
             PTP_InitStruct->PTP_PtpEnable);
             
  ETHx->PTPPSCR = (PTP_InitStruct->PTP_PpsFreSelection & 0x0000000F); 
  ETHx->PTPTTHR = PTP_InitStruct->PTP_TargetTimeHigh;
  ETHx->PTPTTLR = PTP_InitStruct->PTP_TargetTimeLow;
  ETHx->PTPTSCR = tmpreg;
  ETHx->PTPSSIR = PTP_InitStruct->PTP_SystemTimeSubseconds;
  ETHx->PTPTSHUR = PTP_InitStruct->PTP_UpdateRegisterHigh;
  ETHx->PTPTSLUR = PTP_InitStruct->PTP_UpdateRegisterLow;


  ETHx->PTPTSAR = PTP_InitStruct->PTP_Addend;
  ETHx->PTPTSCR |= (0x00000001 << 5);
  while(ETHx->PTPTSCR & (0x00000001 << 5));
  ETHx->PTPTSCR |= (0x00000001 << 2);
  while(ETHx->PTPTSCR & (0x00000001 << 2));
  ETHx->PTPTSCR |= PTP_SystemTimeInit_Enable;  
