...
//extern void lwip_packet_eht0_handler(void);
...
ETH_DMADESCTypeDef DMARxDscrTabEth1[ETH_RXBUFNB] __attribute__((aligned(4))) = {0};
ETH_DMADESCTypeDef DMATxDscrTabEth1[ETH_TXBUFNB] __attribute__((aligned(4))) = {0};
uint8_t Rx_BuffEth1[ETH_RX_BUF_SIZE*ETH_RXBUFNB] __attribute__((aligned(4))) = {0};
uint8_t Tx_BuffEth1[ETH_TX_BUF_SIZE*ETH_TXBUFNB] __attribute__((aligned(4))) = {0};
#endif
...
uint8_t LAN8720_Get_Speed(void)
{
	...
	speed=((ETH_ReadPHYRegister(ETH0, 0x00,31)&0x1C) >> 2); 
	return speed;
}   
...
uint8_t ETH_MACDMA_Config(void)
{
  ...
  ETH_DeInit(ETH0);  							
  ETH_SoftwareReset(ETH0);  				
  while (ETH_GetSoftwareResetStatus(ETH0) == SET);
  ...
  ETH_InitStructure.ETH_DropTCPIPChecksumErrorFrame = ETH_DropTCPIPChecksumErrorFrame_Enable;
	...
	rval=ETH_Init(ETH0,&ETH_InitStructure,LAN8720_PHY_ADDRESS);

	if(rval==ETH_SUCCESS)
	{
		ETH_DMAINTConfig(ETH0, ETH_DMA_INT_NIS|ETH_DMA_INT_R,ENABLE);
	}

#if defined (LWIP_MULTI_MAC)  
  ETH_DeInit(ETH1);  							
  ETH_SoftwareReset(ETH1);  				
  while (ETH_GetSoftwareResetStatus(ETH1) == SET);
  
  rval=ETH_Init(ETH1,&ETH_InitStructure,LAN8720_PHY_ADDRESS);

	if(rval==ETH_SUCCESS)
	{
		ETH_DMAINTConfig(ETH1, ETH_DMA_INT_NIS|ETH_DMA_INT_R,ENABLE);
	}
#endif
...
void MAC0_SBD_IRQHandler(void)
{
  ...
  ETH_DMAClearINTPendingBit(ETH0, ETH_DMA_INT_R);
  ETH_DMAClearINTPendingBit(ETH0, ETH_DMA_INT_AIS);
  ETH_DMAClearINTPendingBit(ETH0, ETH_DMA_INT_NIS); 
}
...
{
	while(ETH_GetRxPktSize(DMARxDescToGetEth1)!=0) 
	{ 	       
		lwip_packet_eth1_handler();
	} 
    

  ETH_DMAClearINTPendingBit(ETH1, ETH_DMA_INT_R);
  ETH_DMAClearINTPendingBit(ETH1, ETH_DMA_INT_AIS);
  ETH_DMAClearINTPendingBit(ETH1, ETH_DMA_INT_NIS); 
}
#endif
...
{ 
	...
	{	
		...
		if ((ETH0->DMASR&ETH_DMASR_RBUS)!=(uint32_t)RESET)  
		{ 
			ETH0->DMASR = ETH_DMASR_RBUS;
			ETH0->DMARPDR=0;
		}
		return frame;
	}  
...
{ 
	...
	if((DMARxDescToGetEth1->Status&ETH_DMARxDesc_OWN)!=(uint32_t)RESET)
	{	
		frame.length=ETH_ERROR; 
	...
	if(((DMARxDescToGetEth1->Status&ETH_DMARxDesc_ES)==(uint32_t)RESET)&& 
	((DMARxDescToGetEth1->Status & ETH_DMARxDesc_LS)!=(uint32_t)RESET)&&  
	((DMARxDescToGetEth1->Status & ETH_DMARxDesc_FS)!=(uint32_t)RESET))  
	{       
		framelength=((DMARxDescToGetEth1->Status&ETH_DMARxDesc_FL)>>ETH_DMARxDesc_FrameLengthShift)-4;
 		frame.buffer = DMARxDescToGetEth1->Buffer1Addr;
	}
  else
  {
    framelength=ETH_ERROR;
	...
	frame.length=framelength; 
	frame.descriptor=DMARxDescToGetEth1;  

	DMARxDescToGetEth1=(ETH_DMADESCTypeDef*)(DMARxDescToGetEth1->Buffer2NextDescAddr);  
...
{ 
  ...
  DMATxDescToSet->Status|=ETH_DMATxDesc_OWN; // Sets the OWN bit of the Tx descriptor,Buff owned by dma
	...
	if((ETH0->DMASR&ETH_DMASR_TBUS)!=(uint32_t)RESET) // Wthen Tx Buffer TBUS bit was setting ,reset.Restore transport
	{ 
		ETH0->DMASR=ETH_DMASR_TBUS; // Reset the ETH DMA TBUS
		ETH0->DMATPDR=0; // Restore the DMA send
	} 
	else
	{
...
// return:ETH_ERROR
//		ETH_SUCCESS
uint8_t ETH1_Tx_Packet(uint16_t FrameLength)
{ 
	DMATxDescToSetEth1->Status &=~ ETH_DMATxDesc_OWN;
	// Check the current descriptor
	if((DMATxDescToSetEth1->Status&ETH_DMATxDesc_OWN)!=(uint32_t)RESET)
  {
    return ETH_ERROR;
  }
  
  DMATxDescToSetEth1->ControlBufferSize = (FrameLength & ETH_DMATxDesc_TBS1);
  DMATxDescToSetEth1->Status|=ETH_DMATxDesc_LS|ETH_DMATxDesc_FS;
  DMATxDescToSetEth1->Status|=ETH_DMATxDesc_OWN; // Sets the OWN bit of the Tx descriptor,Buff owned by dma
    
		/* Enable the DMA transmission */
	if((ETH1->DMASR&ETH_DMASR_TBUS)!=(uint32_t)RESET) // Wthen Tx Buffer TBUS bit was setting ,reset.Restore transport
	{ 
		ETH1->DMASR=ETH_DMASR_TBUS; // Reset the ETH DMA TBUS
		ETH1->DMATPDR=0; // Restore the DMA send
	} 
	else
	{
...
//   USER_ETH->DMATPDR=0;
  ...
  DMATxDescToSetEth1=(ETH_DMADESCTypeDef*)(DMATxDescToSetEth1->Buffer2NextDescAddr);   
	return ETH_SUCCESS;  
}
#endif
...
{  
  return DMATxDescToSetEth1->Buffer1Addr;
}
#endif
