                                          ...
                                          USBH_EpDesc_TypeDef * epDesc)
{
  ...
  {
    printf("%s", (char *)MSG_MSC_CLASS);
  }
  else if ((*id).bInterfaceClass == 0x03)
  {
    printf("%s", (char *)MSG_HID_CLASS);
  }
}
...
int USBH_USR_MSC_Application(void)
{
  ...
  uint8_t writeTextBuff[] =
    "Connectivity line Host Demo application using FAT_FS   ";
  ...
  case USH_USR_FS_DRAW:
    /* Write file done, while loop when device in connected state */
