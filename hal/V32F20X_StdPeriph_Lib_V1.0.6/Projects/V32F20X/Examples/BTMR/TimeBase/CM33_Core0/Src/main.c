...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
BTMR_InitType BTMR_TimeBaseStructure = {0};
...
int main(void)
{
  ...
  BTMR_DeInit(BTMR0);
  BTMR_StructInit(&BTMR_TimeBaseStructure);
  BTMR_TimeBaseStructure.Period = 20000;
  BTMR_TimeBaseStructure.ExtClk = BTMR_CLKSRC_INTERNAL;
  BTMR_TimeBaseStructure.ExtEn = BTMR_EXTEN_DISABLE;
  BTMR_Init(BTMR0, &BTMR_TimeBaseStructure);

  BTMR_INTConfig(BTMR0, ENABLE);
  
  BTMR_Cmd(BTMR0, ENABLE);
  
  printf("Time base init success\r\n");
  while (1)
  {
  }
}
...
void RCC_Configuration(void)
{
  ...
  SYSCFG0_TMRClockCmd(SYSCFG0_TMRCLK_BTMR_BPWM_GLOBAL | SYSCFG0_TMRCLK_BTMR0, ENABLE);
}
