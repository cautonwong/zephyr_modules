...
/* Private define ------------------------------------------------------------*/
#define DAC_DHR12RD_ADDRESS        ((uint32_t)&(DAC->DHR12RD))
...
int main(void)
{
  ...
  TMR_TimeBaseStructure.TMR_Period           = 2 - 1;
  ...
  DMA_InitStructure.DMA_SourceAddress             = (uint32_t)&DualSine12bit;
  DMA_InitStructure.DMA_DestinationAddress        = DAC_DHR12RD_ADDRESS;
  ...
  SYSCFG0_DMARequestConfig(SYSCFG0_DMA0_REQ0, SYSCFG0_DMA_REQUEST_DAC_1);
  ...
  DAC_DMACmd(DAC_CHANNEL_0, ENABLE);
  DAC_DMACmd(DAC_CHANNEL_1, ENABLE);

  printf("DAC dual DMA sin wave test\r\n");
  while (1)
  {
  }
}
