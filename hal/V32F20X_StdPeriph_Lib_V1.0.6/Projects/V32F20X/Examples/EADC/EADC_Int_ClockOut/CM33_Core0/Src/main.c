...
int main(void)
{
  ...
  EADC_InitStructure.EADC_ClkOutFreqDiv    = 255;
  EADC_InitStructure.EADC_CdmaClkFreqDiv   = EADC_CDMA_CLK_DIV_1;
  ...
  DMA_InitStructure.DMA_TransferSize              = 256*8;
  ...
  EADC_Cmd(EADC_CLK_OUT, EADC_CHANNEL_0 | EADC_CHANNEL_1 | EADC_CHANNEL_3 | 
  EADC_CHANNEL_4 | EADC_CHANNEL_5 | EADC_CHANNEL_6 | EADC_CHANNEL_7, ENABLE); 

  printf("EADC clock out test\r\n");
  /* Infinite loop */
  while (1)
  {
...
{
  /* EADC CLK = RCH/4 */
  SYSCFG0_EADCClockConfig(SYSCFG0_EADC_CLKSRC_RCH, 1);
