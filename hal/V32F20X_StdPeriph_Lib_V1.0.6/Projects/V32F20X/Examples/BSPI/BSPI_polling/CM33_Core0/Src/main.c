...
static uint32_t Transmit_Buffer_8b[8] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08};
static uint32_t Recieve_Buffer_8b[8] = {};
...
int main(void)
{
  ...
  */
  ...
  printf("BSPI2 enabled\r\n");
  
  memset(Recieve_Buffer_8b, 0, sizeof(Recieve_Buffer_8b));
  for (i=0; i<8; i++)
  {
    /* Loop while until TXFIFO is empty */
    while (RESET == BSPI_GetStatus(BSPI0, SPI_STS_TFE));
    BSPI_SendData(BSPI0, Transmit_Buffer_8b[i]);

    /* While loop until idle */
    while (SET == BSPI_GetStatus(BSPI0, SPI_STS_BSY));
    
    /* While until recieve */
    while (RESET == BSPI_GetStatus(BSPI2, SPI_STS_RNE));
    Recieve_Buffer_8b[i] =  BSPI_ReceiveData(BSPI2);
  }
