...
int main(void)
{
  ...
  /* GTMR0 configuration: PWM Input mode ------------------------
     The external signal is connected to GTMR0 CH2 pin (PA.01), 
     The Rising edge is used as active edge,
     The GTMR0 CCR2 is used to compute the frequency value 
     The GTMR0 CCR1 is used to compute the duty cycle value
  ------------------------------------------------------------ */
  ...
  TMR_PWMIConfig(GTMR0, &TMR_ICInitStructure);

  /* Select the GTMR0 Input Trigger: TI2FP2 */
  TMR_SelectInputTrigger(GTMR0, TMR_TS_TI2FP2);

  /* Select the slave Mode: Reset Mode */
  TMR_SelectSlaveMode(GTMR0, TMR_SlaveMode_Reset);
...
void GPIO_Configuration(void)
{
  ...
  GPIO_PinAFConfig(GPIOA, GPIO_PinSource1, GPIO_AF1_GTMR0_IO2);
