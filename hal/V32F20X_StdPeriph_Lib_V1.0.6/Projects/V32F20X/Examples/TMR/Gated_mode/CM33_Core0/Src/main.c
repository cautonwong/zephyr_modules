...
int main(void)
{
  ...
  /* Gated mode test -----
    ...
    2/ETMR1 is configured as Slave Timer for an external Trigger connected
     to ETMR1 TI2 pin (ETMR1 CH2 configured as input pin):
     - The ETMR1 TI2FP2 is used as Trigger Input
     - Rising edge is used to start and stop the ETMR1: Gated Mode.

    * The TIMxCLK is fixed to 48 MHZ, the Prescaler is equal to 0 so the TMRx clock 
      counter is equal to 48 MHz.
      The Timers are running at: 
  ...
  */
  ...
  TMR_TimeBaseStructure.TMR_Period = 95;
  ...
  /* ETMR1 output mode configuration */
  ...
  /* configure ETMR1 channel2 to get clock signal */
  ...
  /* Select the ETMR1 Input Trigger: TI2FP2 */
  TMR_SelectInputTrigger(ETMR1, TMR_TS_TI2FP2);

  /* Select the slave Mode: External1 Mode */
  TMR_SelectSlaveMode(ETMR1, TMR_SlaveMode_Gated);
