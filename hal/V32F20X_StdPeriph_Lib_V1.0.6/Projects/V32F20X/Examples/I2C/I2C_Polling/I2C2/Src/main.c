...
int main(void)
{
  ...
  I2C_INTConfig(I2C2, I2C_INT_RD_REQ, DISABLE);
  I2C_INTConfig(I2C2, I2C_INT_RD_REQ, DISABLE);
  NVIC_DisableIRQ(FLEXCOMM2_IRQn);
  ...
  if(I2C_mode == 1)
  {
    ...
    **********************************************************/
    ...
    for(i = 0; i < 255; i++)
    {
      while(I2C_GetFlag(I2C2, I2C_FLAG_TFE) == RESET);
      I2C_SendCmdDataFrame(I2C2, I2C_CMD_WRITE, I2C_Send_Data[i]);
    }
    
    /* last data with stop */
    while(I2C_GetFlag(I2C2, I2C_FLAG_TFE) == RESET);
    I2C_SendCmdDataFrame(I2C2, I2C_CMD_WRITE_STOP, I2C_Send_Data[i]);
        
    /* waite for idle */
    while(I2C_GetFlag(I2C2, I2C_FLAG_ACTIVE) == SET);
  }
  else if(I2C_mode == 2)
  {
    /**********************************************************
    ...
    **********************************************************/
    ...
    I2C_SendCmdDataFrame(I2C2, I2C_CMD_READ, 0x00);
    /* start recieve 8 data */
    for(i = 0; i < 6; i++)
    {
      while(I2C_GetFlag(I2C2, I2C_FLAG_RFNE) == RESET);
      I2C_SendCmdDataFrame(I2C2, I2C_CMD_READ, 0x00);
      I2C_Reci_Data[I2C_Reci_count] = I2C_ReceiveData(I2C2);
      I2C_Reci_count += 1;
    }
    
    while(I2C_GetFlag(I2C2, I2C_FLAG_RFNE) == RESET);
    I2C_SendCmdDataFrame(I2C2, I2C_CMD_READ_STOP, 0x00);
    I2C_Reci_Data[I2C_Reci_count] = I2C_ReceiveData(I2C2);
    I2C_Reci_count += 1;
    while(I2C_GetFlag(I2C2, I2C_FLAG_RFNE) == RESET);
    I2C_Reci_Data[I2C_Reci_count] = I2C_ReceiveData(I2C2);
  ...
  else if(I2C_mode == 3)
  {
    ...
    **********************************************************/
    ...
    for(i = 0; i < 5; i++)
    {
      while(I2C_GetFlag(I2C2, I2C_FLAG_SLV_HOLD_TX_FIFO_EMPTY) == RESET);
      I2C_SendCmdDataFrame(I2C2, I2C_CMD_WRITE, I2C_Send_Data[i]);
    }
  ...
  else if(I2C_mode == 5)
  {
    ...
    **********************************************************/
    ...
    while(I2C_GetFlag(I2C2, I2C_FLAG_TFE) == RESET);
    I2C_SendCmdDataFrame(I2C2, I2C_CMD_WRITE, 0x00);
    ...
    {
      while(I2C_GetFlag(I2C2, I2C_FLAG_TFE) == RESET);
      I2C_SendCmdDataFrame(I2C2, I2C_CMD_WRITE, I2C_Send_Data[i]+0x10);
    }
    
    /* last data with stop */
    while(I2C_GetFlag(I2C2, I2C_FLAG_TFE) == RESET);
    I2C_SendCmdDataFrame(I2C2, I2C_CMD_WRITE_STOP, I2C_Send_Data[i]+0x10);
  ...
  else if(I2C_mode == 6)
  {
    ...
    **********************************************************/
    ...
    while(I2C_GetFlag(I2C2, I2C_FLAG_TFE) == RESET);
    I2C_SendCmdDataFrame(I2C2, I2C_CMD_READ_RESTART, 0x00);
