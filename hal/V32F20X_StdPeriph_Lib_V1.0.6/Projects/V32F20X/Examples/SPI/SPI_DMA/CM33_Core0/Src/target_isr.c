...
{
  if (DMA_GetITtatus(DMA0_Channel2, DMA_INT_BLOCK) == SET)
  {
    DMA_SPI_Slave_Send_count += 1;
    DMA_ClearITStatus(DMA0_Channel2, DMA_INT_BLOCK);
  }
}

void DMA0_CH3_IRQHandler(void)
{
  if (DMA_GetITtatus(DMA0_Channel3, DMA_INT_BLOCK) == SET)
  {
    DMA_SPI_Slave_Reci_count += 1;  
    DMA_ClearITStatus(DMA0_Channel3, DMA_INT_BLOCK);
  }
}
