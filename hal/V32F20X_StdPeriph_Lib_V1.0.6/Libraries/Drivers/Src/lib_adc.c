...
#define ADC_EXT_TRIG_BTIMER_EN      ((uint32_t)0x00000010)
#define ADC_EXT_TRIG_JBTIMER_EN     ((uint32_t)0x00000020)
...
/**
  ...
  * @param  ADCx: where x can be 0, 1 to select the ADC peripheral.
...
void ADC_Init(ADC_Type* ADCx, ADC_InitType* ADC_InitStruct)
{
  ...
  assert_parameters(IS_ADC_ALL_PERIPH(ADCx));
  assert_parameters(IS_ADC_MODE(ADC_InitStruct->ADC_Mode));
  assert_parameters(IS_FUNCTIONAL_STATE(ADC_InitStruct->ADC_ScanMode));
  assert_parameters(IS_FUNCTIONAL_STATE(ADC_InitStruct->ADC_ContinuousMode));
  assert_parameters(IS_ADC_EXT_TRIG(ADC_InitStruct->ADC_ExternalTrig));
  assert_parameters(IS_ADC_DATA_ALIGN(ADC_InitStruct->ADC_DataAlign));
  assert_parameters(IS_ADC_REGULAR_LENGTH(ADC_InitStruct->ADC_NumOfChannel));
  assert_parameters(IS_FUNCTIONAL_STATE(ADC_InitStruct->ADC_BtimerTrig));
  ...
  if(DISABLE == ADC_InitStruct->ADC_BtimerTrig)
  {
    tmpreg1 &= ~ADC_EXT_TRIG_BTIMER_EN;
  }
  else
  {
    tmpreg1 |= ADC_EXT_TRIG_BTIMER_EN;
...
void ADC_StructInit(ADC_InitType* ADC_InitStruct)
{
  ...
  ADC_InitStruct->ADC_ExternalTrig = ADC_EXT_TRIG_CONV_NONE;
  /* Initialize the ADC_DataAlign member */
  ADC_InitStruct->ADC_DataAlign = ADC_DataAlign_Right;
  /* Initialize the ADC_NbrOfChannel member */
  ADC_InitStruct->ADC_NumOfChannel = 1;
  ADC_InitStruct->ADC_BtimerTrig = DISABLE;
}
...
void ADC_INTConfig(ADC_Type* ADCx, uint16_t ADC_INT, FunctionalState NewState)
{
  ...
  assert_parameters(IS_ADC_ALL_PERIPH(ADCx));
  assert_parameters(IS_FUNCTIONAL_STATE(NewState));
  assert_parameters(IS_ADC_INT(ADC_INT));
...
void ADC_DiscModeChannelCountConfig(ADC_Type* ADCx, uint8_t Number)
{
  ...
  /* Check the parameters */
  assert_parameters(IS_ADC_ALL_PERIPH(ADCx));
  assert_parameters(IS_ADC_REGULAR_DISC_NUMBER(Number));
...
/**
  ...
  *   This parameter can be one of the following values:
  *     @arg ADC_SampleTime_4_5: Sample time equal to 4.5 cycles
...
void ADC_RegularChannelConfig(ADC_Type* ADCx, uint8_t ADC_Channel, uint8_t Rank, uint8_t ADC_SampleTime)
{
  ...
  /* Check the parameters */
  assert_parameters(IS_ADC_ALL_PERIPH(ADCx));
  assert_parameters(IS_ADC_CHANNEL(ADC_Channel));
  assert_parameters(IS_ADC_REGULAR_RANK(Rank));
  assert_parameters(IS_ADC_SAMPLE_TIME(ADC_SampleTime));
...
/**
  ...
  *   This parameter can be one of the following values:
  *     @arg ADC_EXT_TRIG_INJEC_T1_TRGO: Timer1 TRGO event selected (for ADC0, ADC1 )
  *     @arg ADC_EXT_TRIG_INJEC_T1_CC4: Timer 1 capture compare4 selected (for ADC0, ADC1 )
  *     @arg ADC_EXT_TRIG_INJEC_T3_TRGO: Timer 3 TRGO event selected (for ADC0 and ADC1)
  *     @arg ADC_EXT_TRIG_INJEC_T3_CC1: Timer 3 capture compare1 selected (for ADC0 and ADC1)
  *     @arg ADC_EXT_TRIG_INJEC_T4_CC4: Timer 4 capture compare4 selected (for ADC0 and ADC1)
  *     @arg ADC_EXT_TRIG_INJEC_T5_TRGO: Timer 5 TRGO event selected (for ADC0 and ADC1)
  *     @arg ADC_EXT_TRIG_INJEC_EXT_IT15_TIM2_CC4: External interrupt line 15 or Timer2
  *                                                       capture compare4 event selected (for ADC0 and ADC1)                       
  *     @arg ADC_EXT_TRIG_INJEC_BPWM0_TRGO: Basic PWM0 TRGO event selected (for ADC0 and ADC1)
  *     @arg ADC_EXT_TRIG_INJEC_BPWM1_TRGO: Basic PWM1 TRGO event selected (for ADC0 and ADC1)                        
  *     @arg ADC_EXT_TRIG_INJEC_BPWM2_TRGO: Basic PWM2 TRGO event selected (for ADC0 and ADC1)
  *     @arg ADC_EXT_TRIG_INJEC_BPWM3_TRGO: Basic PWM3 TRGO event selected (for ADC0 and ADC1)                        
  *     @arg ADC_EXT_TRIG_INJEC_NONE: Injected conversion started by software and not
  *                                          by external trigger (for ADC0, ADC1)
  * @param  BtimerTrig: Whether to use Btimer trigger.
  *   This parameter can be set to ENABLE or DISABLE 
  * @retval None
  */
void ADC_ExternalTrigInjectedConvConfig(ADC_Type* ADCx, uint32_t ADC_ExternalTrigInjecConv, FunctionalState BtimerTrig)
{
  uint32_t tmpreg = 0;
  /* Check the parameters */
  assert_parameters(IS_ADC_ALL_PERIPH(ADCx));
  assert_parameters(IS_ADC_EXT_INJEC_TRIG(ADC_ExternalTrigInjecConv));
  assert_parameters(IS_FUNCTIONAL_STATE(BtimerTrig));
  ...
  if (DISABLE == BtimerTrig)
  {
    tmpreg &= ~ADC_EXT_TRIG_JBTIMER_EN;
  }
  else
  {
    tmpreg |= ADC_EXT_TRIG_JBTIMER_EN;
...
void ADC_InjectedChannelConfig(ADC_Type* ADCx, uint8_t ADC_Channel, uint8_t Rank, uint8_t ADC_SampleTime)
{
  ...
  assert_parameters(IS_ADC_ALL_PERIPH(ADCx));
  assert_parameters(IS_ADC_CHANNEL(ADC_Channel));
  assert_parameters(IS_ADC_INJECTED_RANK(Rank));
  assert_parameters(IS_ADC_SAMPLE_TIME(ADC_SampleTime));
...
void ADC_InjectedSequencerLengthConfig(ADC_Type* ADCx, uint8_t Length)
{
  ...
  /* Check the parameters */
  assert_parameters(IS_ADC_ALL_PERIPH(ADCx));
  assert_parameters(IS_ADC_INJECTED_LENGTH(Length));
...
void ADC_SetInjectedOffset(ADC_Type* ADCx, uint8_t ADC_InjectedChannel, uint16_t Offset)
{
  ...
  /* Check the parameters */
  assert_parameters(IS_ADC_ALL_PERIPH(ADCx));
  assert_parameters(IS_ADC_INJECTED_CHANNEL(ADC_InjectedChannel));
  assert_parameters(IS_ADC_OFFSET(Offset));  
...
void ADC_AnalogWDGCtrl(ADC_Type* ADCx, uint32_t ADC_AnalogWatchdog)
{
  ...
  /* Check the parameters */
  assert_parameters(IS_ADC_ALL_PERIPH(ADCx));
  assert_parameters(IS_ADC_ANALOG_WDG(ADC_AnalogWatchdog));
                                   ...
                                   uint16_t LowThreshold)
{
  /* Check the parameters */
  assert_parameters(IS_ADC_ALL_PERIPH(ADCx));
  assert_parameters(IS_ADC_THRESHOLD(HighThreshold));
  assert_parameters(IS_ADC_THRESHOLD(LowThreshold));
...
FlagStatus ADC_GetFlagStatus(ADC_Type* ADCx, uint8_t ADC_FLAG)
{
  ...
  /* Check the parameters */
  assert_parameters(IS_ADC_ALL_PERIPH(ADCx));
  assert_parameters(IS_ADC_GET_FLAG(ADC_FLAG));
...
void ADC_ClearFlag(ADC_Type* ADCx, uint8_t ADC_FLAG)
{
  /* Check the parameters */
  assert_parameters(IS_ADC_ALL_PERIPH(ADCx));
  assert_parameters(IS_ADC_CLEAR_FLAG(ADC_FLAG));
...
/**
  ...
  */
INTStatus ADC_GetINTStatus(ADC_Type* ADCx, uint16_t ADC_INT)
{
  ITStatus bitstatus = RESET;
 
  /* Check the parameters */
  assert_parameters(IS_ADC_ALL_PERIPH(ADCx));
  assert_parameters(IS_ADC_GET_INT(ADC_INT));
  


  /* Check the status of the specified ADC interrupt */
  if ((ADCx->STS & ADC_INT) != (uint32_t)RESET)
  {
    /* ADC_INT is set */
    bitstatus = SET;
  }
...
/**
  ...
  *     @arg ADC_INT_AWD: Analog watchdog interrupt mask
  *     @arg ADC_INT_JEOC: End of injected conversion interrupt mask
  * @retval None
  */
