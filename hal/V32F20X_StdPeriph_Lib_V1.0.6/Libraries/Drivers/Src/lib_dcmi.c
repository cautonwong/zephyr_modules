...
void DCMI_Init(DCMI_InitTypeDef* DCMI_InitStruct)
{
  ...
  assert_parameters(IS_DCMI_CAPTURE_MODE(DCMI_InitStruct->DCMI_CaptureMode));
  assert_parameters(IS_DCMI_SYNCHRO(DCMI_InitStruct->DCMI_SynchroMode));
  assert_parameters(IS_DCMI_PCKPOLARITY(DCMI_InitStruct->DCMI_PCKPolarity));
  assert_parameters(IS_DCMI_VSPOLARITY(DCMI_InitStruct->DCMI_VSPolarity));
  assert_parameters(IS_DCMI_HSPOLARITY(DCMI_InitStruct->DCMI_HSPolarity));
  assert_parameters(IS_DCMI_CAPTURE_RATE(DCMI_InitStruct->DCMI_CaptureRate));
  assert_parameters(IS_DCMI_EXTENDED_DATA(DCMI_InitStruct->DCMI_ExtendedDataMode));
...
{
  /* Check the parameters */
  assert_parameters(IS_DCMI_CONFIG_IT(DCMI_IT));
  assert_parameters(IS_FUNCTIONAL_STATE(NewState));
...
FlagStatus DCMI_GetFlagStatus(uint16_t DCMI_FLAG)
{
  ...
  /* Check the parameters */
  assert_parameters(IS_DCMI_GET_FLAG(DCMI_FLAG));
...
{
  /* Check the parameters */
  assert_parameters(IS_DCMI_CLEAR_FLAG(DCMI_FLAG));
...
ITStatus DCMI_GetITStatus(uint16_t DCMI_IT)
{
  ...
  /* Check the parameters */
  assert_parameters(IS_DCMI_GET_IT(DCMI_IT));
