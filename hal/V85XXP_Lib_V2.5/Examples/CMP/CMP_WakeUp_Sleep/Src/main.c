...
int main(void)
{
  ...
  InitStruct.SignalSourceSel = CMP_SIGNALSRC_PBAT_TO_NPIN; /* Compare BATRTC to N */
  CMP_Init(CMP_2, &InitStruct);

  CMP_CountStructInit(&CountInitStruct);
  CountInitStruct.ModeSel = CMP_MODE_RISING; /* Comparator2 mode rising, ouput 0->1, generate wake-up signal */
  ...
  INTInitStruct.INTNumSel = CMP_INTNUM_1; /* Every time a subtraction is made, an interrupt is generated */
  INTInitStruct.SubSel = CMP_COUNT_SUB; /* If the Comparator2 counter exceeds the \
                                           CMP2_THR, the CMP2_THR is subtracted until \
                                           the Comparator2 counter is less than the CMP2_THR*/
  INTInitStruct.THRNum = 5;  /* Every 6 interrupts will be triggered */
  CMP_INTInit(CMP_2, &INTInitStruct);
  
  /* Clear Comparator2 interrupt flag */
  CMP_ClearINTStatus(CMP_2);
  /* Enable Comparator2 interrupt, ANA NVIC interrupt */
  ...
  while(1)
  {
    ...
    Stdio_Init();
#ifdef __GNUC__
    printf("Exit sleep mode, CMP2 CMPCNT: 0x%lx\r\n", (uint32_t)ANA->CMPCNT2);
#else
    printf("Exit sleep mode, CMP2 CMPCNT: 0x%x\r\n", (uint32_t)ANA->CMPCNT2);
#endif
    WDT_Clear();
  }
