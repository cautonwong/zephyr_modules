...
extern uint32_t nTXLen;
extern uint8_t RX_Buffer[4];
extern uint32_t nRXLen;
/**
...
{
  if(UART_GetINTStatus(UART0,UART_INTSTS_TXDONE))
  {
    UART_ClearINTStatus(UART0,UART_INTSTS_TXDONE);
    
    if(nTXLen<4)
    {
      UART_SendData(UART0, TX_Buffer[nTXLen]);
      nTXLen++;
    }
  }
  if(UART_GetINTStatus(UART0,UART_INTSTS_RX))
  {
    UART_ClearINTStatus(UART0,UART_INTSTS_RX);
    
    if(nRXLen<4)
    {
      RX_Buffer[nRXLen] = UART_ReceiveData(UART0);
      nRXLen++;
    }
  }
}
