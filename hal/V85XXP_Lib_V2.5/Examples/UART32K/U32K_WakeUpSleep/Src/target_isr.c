...
{
 /* Receive handling */
  if (U32K_GetINTStatus(U32K0, U32K_INTSTS_RX))
  {
    U32K_ClearINTStatus(U32K0, U32K_INTSTS_RX);
    Receive_Buffer[Receive_Cnt++] = U32K_ReceiveData(U32K0);
    /* Disable receive interrupt when receive 0xFE */
    if (Receive_Buffer[Receive_Cnt - 1] == 0xFE)
    {
      U32K_INTConfig(U32K0, U32K_INT_RX, DISABLE);
      Receive_Done = 1;
    }
  }
  /* Error handling */
  if (U32K_GetINTStatus(U32K0, U32K_INTSTS_RXOV) || U32K_GetINTStatus(U32K0, U32K_INTSTS_RXPE))
  {
    __NOP();
    //Add Error handler
  }
