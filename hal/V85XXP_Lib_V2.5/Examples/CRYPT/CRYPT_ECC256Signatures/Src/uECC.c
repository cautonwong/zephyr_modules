...
#define HW_CRYPT_ACC  /* Hardware Crypto accelerator */
//#define TOGGLE_IO   /* Toggle IO for debug */
...
//    uECC_word_t result[16], i;

#ifdef HW_CRYPT_ACC
    
    CRYPT->PTRA = (unsigned int)&vli[0];
    CRYPT->PTRO = (unsigned int)&vli[0];
//    CRYPT->PTRO = (unsigned int)&result[0];
    CRYPT->CTRL = ((num_words-1)<<8)|0x31;
    
    while (CRYPT->CTRL & 0x1);
...
#ifdef HW_CRYPT_ACC    
    uECC_word_t tmp, ii;
    uECC_word_t tmp_buf[16];
//    uECC_word_t result2[16];

    tmp = (unsigned int)&left[0];
    if ((tmp&0xF0000000) == 0) {
      for (ii=0;ii<num_words;ii++)
        tmp_buf[ii] = left[ii];
      CRYPT->PTRA = (unsigned int)&tmp_buf[0];
    } else {
      CRYPT->PTRA = tmp;
    }
    tmp = (unsigned int)&right[0];
    if ((tmp&0xF0000000) == 0) {
      for (ii=0;ii<num_words;ii++)
        tmp_buf[ii] = right[ii];
      CRYPT->PTRB = (unsigned int)&tmp_buf[0];
    } else {
      CRYPT->PTRB = tmp;
    }
    CRYPT->PTRO = (unsigned int)&result[0];
    CRYPT->CTRL = ((num_words-1)<<8)|0x11;
...
//      }
//    }
//    if (carry != *CRYPT_CARRY)
//      print1("ADD Carry Fail!!!\n");
...
//      }
//    }
//    if (borrow != *CRYPT_CARRY)
//      print1("SUB Carry Fail!!!\n");
...
//    uECC_word_t result2[32];
    ...
    CRYPT->PTRA = (unsigned int)&left[0];
    CRYPT->PTRB = (unsigned int)&left[0];
    CRYPT->PTRO = (unsigned int)&result[0];
    CRYPT->CTRL = ((num_words-1)<<8)|0x1;
...
//    uECC_word_t result2[32], borrow2, j;
    
    for (index = 1; shift >= 0; --shift) {
        uECC_word_t borrow = 0;
#ifdef HW_CRYPT_ACC
        borrow = uECC_vli_sub(&v[1 - index][0], &v[index][0], mod_multiple, num_words*2);
#else        
        wordcount_t i;
        for (i = 0; i < num_words * 2; ++i) {
...
#ifdef HW_CRYPT_ACC
        uECC_vli_rshift1(mod_multiple, num_words*2);
#else
