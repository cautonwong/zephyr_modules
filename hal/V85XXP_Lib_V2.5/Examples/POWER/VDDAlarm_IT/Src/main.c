...
int main(void)
{
  ...
  /*VDD alarm threshole is 3.6v, Check every 30us */
  PMU_VDDAlarmTHConfig(PMU_VDDALARM_3_6V);
  PMU_VDDDetectorCmd(PMU_VDDDET_ON_RTCCLK);

  /* Clear VDDALARM interrupt flag */
  ANA_ClearINTStatus(ANA_INT_VDDALARM);
  /* Enable VDDALARM interrupt, ANA NVIC interrupt */
  ANA_INTConfig(ANA_INT_VDDALARM, ENABLE);
