...
{
  if (UART_GetINTStatus(UART5, UART_INTSTS_TXOV))
  {
    flag_uarterr = 1;
    UART_ClearINTStatus(UART5, UART_INTSTS_TXOV);
  }
}
...
{
  if (DMA_GetINTStatus(DMA_INTSTS_C2FE))
  {
    flag_framend = 1;
    DMA_ClearINTStatus(DMA_INTSTS_C2FE);
  }
}
