...
extern __IO uint16_t CapData[10];
...
{
  /* Error(capture overflow) handle */
  if(PWM_GetChannelINTStatus(PWM3, PWM_CHANNEL_0, PWM_INT_COV))
  {
    PWM_ClearChannelINTStatus(PWM3, PWM_CHANNEL_0, PWM_INT_COV);
    printf("Capture overflow!\r\n");
  }

  /* Capture handle */
  if(PWM_GetChannelINTStatus(PWM3, PWM_CHANNEL_0, PWM_INT_CCIFG))
  {
    PWM_ClearChannelINTStatus(PWM3, PWM_CHANNEL_0, PWM_INT_CCIFG);
    CapData[count++] = PWM_GetCapture(PWM3, PWM_CHANNEL_0);
  }
}
