...
void LCD_Cmd(LCD_IOInitType *IOInitType, uint32_t NewState)
{
  ...
  assert_parameters(IS_LCD_TYPE(IOInitType->COMMode));

  tmp = LCD->CTRL;
  tmp &= ~LCD_CTRL_TYPE;
  tmp |= IOInitType->COMMode;
  LCD->CTRL = tmp;
  
  if (NewState == ENABLE)
  {
    /* Rising edge of LCD_CTRL_EN */
    LCD->CTRL &= ~LCD_CTRL_EN;
    ...
    *(lcd_comio[3].GPIO+1) &= ~lcd_comio[3].Pin;
    if (IOInitType->COMMode == LCD_TYPE_6COM)
    {
    ...
    else if (IOInitType->COMMode == LCD_TYPE_8COM)
    {
      *lcd_comio[4].GPIO |= lcd_comio[4].Pin;
      *(lcd_comio[4].GPIO+1) &= ~lcd_comio[4].Pin;
      *lcd_comio[5].GPIO |= lcd_comio[5].Pin;
  ...
  else
  {
    ...
    *lcd_comio[3].GPIO &= ~lcd_comio[3].Pin;
    ...
    if (IOInitType->COMMode == LCD_TYPE_8COM)
    {
      *(lcd_comio[4].GPIO+2) &= ~lcd_comio[4].Pin;
      *lcd_comio[4].GPIO &= ~lcd_comio[4].Pin;
      *(lcd_comio[5].GPIO+2) &= ~lcd_comio[5].Pin;
...
void LCD_VoltageConfig(uint32_t VLCDSelection)
{
  ...
  /* Check parameters */
  assert_parameters(IS_LCD_VOL(VLCDSelection));
