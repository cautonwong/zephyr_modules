...
{
  uint32_t tmp, tmp_cmpctl;
  uint32_t hclk;

  /* Check parameters */  
  assert_parameters(IS_PMU_COMP1PWR(InitStruct->CMP1Power));
  assert_parameters(IS_PMU_COMP2PWR(InitStruct->CMP2Power));
  ...
  ANA->REG3 |= InitStruct->CMP1Power;
  
  /******** Comparator 2 power control ********/
  ANA->REG3 &= ~ANA_REG3_CMP2PDN;
  ANA->REG3 |= InitStruct->CMP2Power;
  ...
  tmp_cmpctl = ANA->CMPCTL;
  tmp_cmpctl &= ~ANA_CMPCTL_VDDALARM_CHK_FRQ_SEL;
  tmp = ANA->REG9;
  if (InitStruct->VDDDetector == PMU_VDDDET_ALWAYSON)
  {
    tmp &= ~ANA_REG9_VDDDETPD;
  }
  else if (InitStruct->VDDDetector == PMU_VDDDET_OFF)
  {
    tmp |= ANA_REG9_VDDDETPD;
  }
  else if (InitStruct->VDDDetector == PMU_VDDDET_ON_RTCCLK)
  {
    tmp |= ANA_REG9_VDDDETPD;
    tmp_cmpctl |= (1UL << ANA_CMPCTL_VDDALARM_CHK_FRQ_SEL_Pos);
  }
  else
  {
    tmp |= ANA_REG9_VDDDETPD;
    tmp_cmpctl |= (2UL << ANA_CMPCTL_VDDALARM_CHK_FRQ_SEL_Pos);
  }
  ANA->REG9 = tmp;
  ANA->CMPCTL = tmp_cmpctl;
...
/**
  ...
  * @retval None
  */
void PMU_VDDAlarmTHConfig(uint32_t Threshold)
{
  uint32_t tmp;
  
  /* Check parameters */ 
  assert_parameters(IS_PMU_VDDALARM_THR(Threshold));
...
/**
  ...
  * @retval None
  */
void PMU_VDDDetectorCmd(uint32_t VDDNewState)
{
  uint32_t tmp_ana9, tmp_cmpctl;
  
  /* Check parameters */ 
  assert_parameters(IS_PMU_VDDDET(VDDNewState));
  
  tmp_cmpctl = ANA->CMPCTL;
  tmp_cmpctl &= ~ANA_CMPCTL_VDDALARM_CHK_FRQ_SEL;
  tmp_ana9 = ANA->REG9;
  if (VDDNewState == PMU_VDDDET_ALWAYSON)
  {
    tmp_ana9 &= ~ANA_REG9_VDDDETPD;
  }
  else if (VDDNewState == PMU_VDDDET_OFF)
  {
    tmp_ana9 |= ANA_REG9_VDDDETPD;
  }
  else if (VDDNewState == PMU_VDDDET_ON_RTCCLK)
  {
    tmp_ana9 |= ANA_REG9_VDDDETPD;
    tmp_cmpctl |= (1UL << ANA_CMPCTL_VDDALARM_CHK_FRQ_SEL_Pos);
  }
...
/**
  * @brief  Selects VDCIN hysteresis.
  * @param  HYSSEL:
                PMU_VDCINHYSSEL_100MV
                PMU_VDCINHYSSEL_200MV
  * @retval None
  */
void PMU_VDCINHYSConfig(uint32_t HYSSEL)
{
  uint32_t tmp;
  /* Check parameters */ 
  assert_parameters(IS_PMU_VDCIN_HYSSEL(HYSSEL));
  
  tmp = ANA->REG7;
  tmp &= ~ANA_REG7_VDCINHYSSEL;
  tmp |= HYSSEL;
  ANA->REG7 = tmp;
}
