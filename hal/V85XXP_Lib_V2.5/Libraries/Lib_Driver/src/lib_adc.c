...
#define ANA_REG1_RSTValue           (0UL)
#define ANA_REG11_RSTValue          (0UL)

#define ADC_SYNC_WR(); {__NOP(); __NOP(); __NOP(); __NOP();}

#define ADC_TIMEOUT_VALUE           (50000UL)
ADC_GetTimeoutFlag_t adc_timeout_cb = 0;
...
/**
  ...
  *                  - neq 0  Function failed.
  */
uint32_t ADC_DeInit(void)
{
  uint32_t tmp[3];
  uint32_t ANA_ADCCTRL0_RSTValue = (0x300000U);
  uint32_t ANA_ADCCTRL1_RSTValue = (0x02U);
  uint32_t ANA_ADCCTRL2_RSTValue = (0x8010U);
  uint32_t ANA_ADCDATATHD1_0_RSTValue = (0U);
  uint32_t ANA_ADCDATATHD3_2_RSTValue = (0U);
  uint32_t ANA_ADCDATATHD_CH_RSTValue = (0U);
  uint32_t RTC_ADCUCALK_RSTValue = (0x599A599AU);
  uint32_t RTC_ADCMACTL_RSTValue = (0x70000000U);
  uint32_t RTC_ADCDTCTL_RSTValue = (0x80000000U);
  ...
  if (need_check)
  {
    for(i=0; i<nCount; i++)
    {
      ...
      if(nAddress == (uint32_t)(&ANA->ADCCTRL0))
      {
        ANA_ADCCTRL0_RSTValue = nValue;
      }
      else if(nAddress == (uint32_t)(&ANA->ADCCTRL1))
      {
        ANA_ADCCTRL1_RSTValue = nValue;
      }
      else if(nAddress == (uint32_t)(&ANA->ADCCTRL2))
      {
        ANA_ADCCTRL2_RSTValue = nValue;
      }
      else if(nAddress == (uint32_t)(&ANA->ADCDATATHD1_0))
      {
        ANA_ADCDATATHD1_0_RSTValue = nValue;
      }
      else if(nAddress == (uint32_t)(&ANA->ADCDATATHD3_2))
      {
        ANA_ADCDATATHD3_2_RSTValue = nValue;
      }
      else if(nAddress == (uint32_t)(&ANA->ADCDATATHD_CH))
      {
        ANA_ADCDATATHD_CH_RSTValue = nValue;
      }
      else if(nAddress == (uint32_t)(&RTC->ADCUCALK))
      {
        RTC_ADCUCALK_RSTValue = nValue;
      }
      else if(nAddress == (uint32_t)(&RTC->ADCMACTL))
      {
        RTC_ADCMACTL_RSTValue = nValue;
      }
      else if(nAddress == (uint32_t)(&RTC->ADCDTCTL))
      {
        RTC_ADCDTCTL_RSTValue = nValue;
      }
      else
      { /* No operation */ }
    } 
...
/**
  ...
  * @note   ADC calibration is implemented when DPORST or ADC RESET happened.
  * @param  None
...
{
  uint32_t timeout_flag = 0UL;
  uint32_t timeout_cnt = 0UL;
  ...
  /* Reset ADC */
  ANA->ADCCTRL2 |= ANA_ADCCTRL2_RESET;
  ADC_SYNC_WR();
  ANA->ADCCTRL2 &= ~ANA_ADCCTRL2_RESET;
  ADC_SYNC_WR();
  ...
  {    
    /* Check adc CAL done */
    if((ANA->ADCCTRL2 & ANA_ADCCTRL2_ADC_CAL_DONE))
    {
      /* Disable ADC TRG_CAL */
      ANA->ADCCTRL2 &= ~ANA_ADCCTRL2_ADC_EN_TRG_CAL;
      ADC_SYNC_WR();
    ...
    if ((uint32_t)adc_timeout_cb == 0UL)
    {
      timeout_cnt++;
      if (timeout_cnt > ADC_TIMEOUT_VALUE)
        timeout_flag = 1UL;
    }
    else
    {
      timeout_flag = adc_timeout_cb();
    }
...
/**
  * @brief  Starts ADC manual-trigger.
  * @param  None
  * @retval None
  */
...
/**
  ...
  */
uint8_t ADC_WaitForManual(uint32_t Timeout)
{
  uint32_t timeout_cnt = 0UL;
  ...
  {    
    /* Check adc done */
    if(0 == (ANA->ADCCTRL0 & ANA_ADCCTRL0_MTRIG))
    {
      return 0;
    }
...
/**
  ...
  * @param  ADC init values
  * @retval 0: function succeeded.
  *         1: function failed.
  */
uint8_t ADC_SoftReset(ADC_InitType* ADC_InitStruct)
{
  /* ADC Calibration */
...
/**
  * @brief  Waits until the last Auto ADC conversion done.
  * @param  None
  * @retval 0: adc finish 
            1: adc timeout

  */
uint8_t ADC_WaitForAuto(uint32_t Timeout)
{
  uint32_t timeout_cnt = 0UL;
  ...
  {    
    /* Check adc done */
    if(ADC_GetINTStatus(ADC_INTSTS_AUTODONE))
    {
      ADC_ClearINTStatus(ADC_INTSTS_AUTODONE);
      return 0;
    }
    /* Check timeout */
    if (timeout_cnt++ > Timeout)
