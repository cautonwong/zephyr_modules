...
LOG_MODULE_REGISTER(gpio_v32f20x, CONFIG_GPIO_LOG_LEVEL);
...
static int gpio_v32f20x_pm_action(const struct device *dev,
				 enum pm_device_action action)
{
	switch (action) {
	case PM_DEVICE_ACTION_SUSPEND:
		/* Optional: Gate clock if no pins are configured for wake-up */
		break;
...
#define GPIO_V32F20X_INIT(n)                                                   \
        ...
        PM_DEVICE_DT_INST_DEFINE(n, gpio_v32f20x_pm_action);                      \
        DEVICE_DT_INST_DEFINE(n, gpio_v32f20x_init, PM_DEVICE_DT_INST_GET(n),     \
