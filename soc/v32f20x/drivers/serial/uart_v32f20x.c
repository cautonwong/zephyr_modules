...
static int uart_v32f20x_err_check(const struct device *dev)
{
	...
	if (lsr & UART_LSR_OE) err |= UART_ERROR_OVERRUN;
	if (lsr & UART_LSR_PE) err |= UART_ERROR_PARITY;
	if (lsr & UART_LSR_FE) err |= UART_ERROR_FRAMING;
	if (lsr & UART_LSR_BI) err |= UART_BREAK;
...
static int uart_v32f20x_configure(const struct device *dev, const struct uart_config *cfg)
{
	...
	switch (cfg->parity) {
	case UART_CFG_PARITY_NONE: init_struct.Parity = UART_PARITY_NONE; break;
	case UART_CFG_PARITY_ODD:  init_struct.Parity = UART_PARITY_ODD; break;
	case UART_CFG_PARITY_EVEN: init_struct.Parity = UART_PARITY_EVEN; break;
	default: 
	...
	case UART_CFG_STOP_BITS_1:   init_struct.StopBit = UART_STOPBIT_1; break;
	case UART_CFG_STOP_BITS_2:   init_struct.StopBit = UART_STOPBIT_2; break;
	case UART_CFG_STOP_BITS_1_5: init_struct.StopBit = UART_STOPBIT_1_5; break;
	default: 
	...
	case UART_CFG_DATA_BITS_5: init_struct.DataLength = UART_DATALEN_5B; break;
	case UART_CFG_DATA_BITS_6: init_struct.DataLength = UART_DATALEN_6B; break;
	case UART_CFG_DATA_BITS_7: init_struct.DataLength = UART_DATALEN_7B; break;
	case UART_CFG_DATA_BITS_8: init_struct.DataLength = UART_DATALEN_8B; break;
	default: 
	...
	/* Professional Polish: Configure FIFO for high speed reliability */
	UART_FIFOConfig(config->regs, UART_FIFO_RX_1_8_FULL | UART_FIFO_TX_EMPTY);
	UART_FIFOCmd(config->regs, ENABLE);
...
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
static int uart_v32f20x_fifo_fill(const struct device *dev, const uint8_t *tx_data, int len)
{
	const struct uart_v32f20x_config *config = dev->config;
	int num_tx = 0;

	while ((len - num_tx > 0) && (UART_GetFlag(config->regs, UART_FLAG_TXE) != RESET)) {
		UART_SendData(config->regs, tx_data[num_tx++]);
	}
	return num_tx;
}

static int uart_v32f20x_fifo_read(const struct device *dev, uint8_t *rx_data, const int len)
{
	const struct uart_v32f20x_config *config = dev->config;
	int num_rx = 0;

	while ((len - num_rx > 0) && (UART_GetFlag(config->regs, UART_FLAG_RX) != RESET)) {
...
{
	const struct uart_v32f20x_config *config = dev->config;
	return (UART_GetFlag(config->regs, UART_FLAG_TXE) != RESET) && (config->regs->IER & UART_INT_TXE);
}
...
{
	const struct uart_v32f20x_config *config = dev->config;
	return (UART_GetFlag(config->regs, UART_FLAG_RX) != RESET) && (config->regs->IER & UART_INT_RX);
}
...
#define UART_V32F20X_CONFIG_FUNC(n) \
	...
	{ \
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority), \
			    uart_v32f20x_isr, DEVICE_DT_INST_GET(n), 0); \
		irq_enable(DT_INST_IRQN(n)); \
	}
#define UART_V32F20X_IRQ_CFG_FUNC_INIT(n) .irq_config_func = uart_v32f20x_irq_config_##n,
#else
#define UART_V32F20X_CONFIG_FUNC(n)
...
#define UART_V32F20X_INIT(n) \
	...
	DEVICE_DT_INST_DEFINE(n, uart_v32f20x_init, NULL, &uart_v32f20x_data_##n, \
			      ...
			      CONFIG_SERIAL_INIT_PRIORITY, &uart_v32f20x_driver_api);
