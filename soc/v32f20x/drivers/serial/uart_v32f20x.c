...
static int uart_v32f20x_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_SUSPEND:
		...
		if (ret == -ENOENT) ret = 0;
		// Vango HAL: Use Enable to control UART
		// UART_Enable(config->regs, DISABLE);
		clock_control_off(config->clock_dev, config->clock_subsys);
		break;
	case PM_DEVICE_ACTION_RESUME:
		clock_control_on(config->clock_dev, config->clock_subsys);
		ret = pinctrl_apply_state(config->pincfg, PINCTRL_STATE_DEFAULT);
		// UART_Enable(config->regs, ENABLE);
		break;
...
static int uart_v32f20x_init(const struct device *dev) {
   ...
   */
#if defined(V32F20XXX_CM0_CORE) /* Check if CM0 core */
  ...
  /* Hardware Manual Compliance: Configure Flexcomm mode if it's a Flexcomm instance */
  if ((uint32_t)config->regs < 0x41000000) {
...
static int uart_v32f20x_async_tx(const struct device *dev, const uint8_t *buf,
  ...
  dma_cfg.dma_slot = config->tx_dma_channel; /* Request ID */
...
#define UART_V32F20X_INIT(n)                                                   \
  ...
  PM_DEVICE_DT_INST_DEFINE(n, uart_v32f20x_pm_action); \
  DEVICE_DT_INST_DEFINE(n, uart_v32f20x_init, PM_DEVICE_DT_INST_GET(n), &uart_v32f20x_data_##n,    \
