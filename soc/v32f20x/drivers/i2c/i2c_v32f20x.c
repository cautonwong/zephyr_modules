...
LOG_MODULE_REGISTER(i2c_v32f20x, CONFIG_I2C_LOG_LEVEL);
...
static int i2c_v32f20x_pm_action(const struct device *dev, enum pm_device_action action)
{
    const struct i2c_v32f20x_config *config = dev->config;
    int ret;
...
static int i2c_v32f20x_init(const struct device *dev)
{
    ...
    ret = i2c_v32f20x_configure(dev, I2C_MODE_CONTROLLER | i2c_map_dt_bitrate(config->bitrate));
    if (ret < 0) return ret;
...
#define I2C_V32F20X_INIT(n)                                                    \
    ...
    PM_DEVICE_DT_INST_DEFINE(n, i2c_v32f20x_pm_action);                        \
    DEVICE_DT_INST_DEFINE(n, i2c_v32f20x_init, PM_DEVICE_DT_INST_GET(n),       \
                          &i2c_v32f20x_data_##n, &i2c_v32f20x_config_##n,      \
                          POST_KERNEL, CONFIG_I2C_INIT_PRIORITY,               \
                          &i2c_v32f20x_driver_api);
